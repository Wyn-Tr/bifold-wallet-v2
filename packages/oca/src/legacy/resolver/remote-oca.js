...
import { ocaBundleStorageDirectory, ocaCacheDataFileName, defaultBundleIndexFileName, defaultBundleLanguage, } from '../../constants';
import { BrandingOverlay, LegacyBrandingOverlay, OverlayBundle } from '../../types';
import { generateColor } from '../../utils';
import { BrandingOverlayType, DefaultOCABundleResolver, OCABundle } from './oca';
var OCABundleQueueEntryOperation;
(function (OCABundleQueueEntryOperation) {
    OCABundleQueueEntryOperation["Add"] = "add";
    OCABundleQueueEntryOperation["Remove"] = "remove";
})(OCABundleQueueEntryOperation || (OCABundleQueueEntryOperation = {}));
export class RemoteOCABundleResolver extends DefaultOCABundleResolver {
    indexFile = {};
    axiosInstance;
    cacheDataFileName = ocaCacheDataFileName;
    indexFileName;
    _indexFileEtag;
    constructor(indexFileBaseUrl, options) {
        super({}, options);
     ...
     */
    prepareBundleQueue = (newIndexFile, oldIndexFile) => {
     ...
     */
    removeFileFromLocalStorage = async (fileName) => {
        const pathToFile = `${this.fileStoragePath()}/${fileName}`;
        try {
     ...
     */
    matchBundleIndexEntry = (identifiers) => {
        ...
        // order matters here.
        const candidates = [schemaId, unqualifiedSchemaId, credentialDefinitionId, unqualifiedCredDefId, templateId].filter((value) => value !== undefined && value !== null && value !== '');
    ...
    async resolve(params) {
        ...
        if (identifier in this.bundles) {
            return Promise.resolve(new OCABundle(this.bundles[identifier], {
                ...this.options,
        ...
        try {
            const bundleData = await this.loadOCABundle(bundlePath);
            const overlayBundle = new OverlayBundle(params.identifiers.credentialDefinitionId ?? '', bundleData[0]);
            const overlay = overlayBundle.overlays.find((overlay) => overlay.type === BrandingOverlayType.Branding01 || overlay.type === BrandingOverlayType.Branding10);
            if (!overlay) {
                overlayBundle.overlays.push(...this.getFallbackBrandingOverlays(overlayBundle.credentialDefinitionId, overlayBundle.captureBase.digest));
            }
     ...
     */
    getFallbackBrandingOverlays(credentialDefinitionId, captureBase) {
        const legacyBrandingOverlay = new LegacyBrandingOverlay(credentialDefinitionId, {
            capture_base: captureBase,
            type: BrandingOverlayType.Branding01,
            background_color: generateColor(credentialDefinitionId),
        });
        const brandingOverlay = new BrandingOverlay(credentialDefinitionId, {
            capture_base: captureBase,
    ...
    async verifyCacheIntegrity(fetchMissing = false) {
