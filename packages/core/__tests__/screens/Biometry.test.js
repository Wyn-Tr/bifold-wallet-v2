...
describe('Biometry Screen', () => {
    ...
    test('toggle is enabled when biometry is available', async () => {
        const { findByTestId, findByText } = render(<BasicAppContext>
        <AuthContext.Provider value={authContext}>
          <Biometry />
        </AuthContext.Provider>
      </BasicAppContext>);
    ...
    test('can toggle off if on', async () => {
        ...
        const { findByTestId, findByText } = render(<StoreProvider initialState={customStore}>
        <BasicAppContext>
          <AuthContext.Provider value={authContext}>
            <Biometry />
          </AuthContext.Provider>
    ...
    test('shows settings popup when permission is UNAVAILABLE', async () => {
        ...
        getSupportedBiometryType.mockResolvedValueOnce(null);
        const { findByTestId, findByText } = render(<StoreProvider initialState={customStore}>
        <BasicAppContext>
          <AuthContext.Provider value={authContext}>
            <Biometry />
