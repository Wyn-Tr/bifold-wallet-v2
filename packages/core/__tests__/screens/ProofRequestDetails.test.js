...
import { ProofRequestType } from '@bifold/verifier';
...
describe('ProofRequestDetails Screen', () => {
    ...
    const renderView = (params) => {
        return render(<BasicAppContext>
        <ProofRequestDetails navigation={navigation} route={{ params: params }}/>
      </BasicAppContext>);
    };
    test('Proof bundle resolver works correctly', async () => {
        ...
        const wrapper = ({ children }) => (<ContainerProvider value={context}>{children}</ContainerProvider>);
        ...
        expect((bundle?.[0].payload.data[0]).requestedAttributes[0].restrictions).toHaveLength(2);
    });
