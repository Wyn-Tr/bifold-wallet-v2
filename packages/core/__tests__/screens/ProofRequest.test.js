...
import { CredentialExchangeRecord, CredentialRole, CredentialState, ProofExchangeRecord, ProofRole, ProofState, } from '@credo-ts/core';
...
describe('displays a proof request screen', () => {
    ...
    describe('with a proof request', () => {
        ...
        test('displays a proof request with one or more predicates not satisfied', async () => {
            ...
            const { getByText, getByTestId } = render(<BasicAppContext>
          <ProofRequest navigation={useNavigation()} proofId={testProofRequest.id}/>
        </BasicAppContext>);
            await waitFor(() => {
                timeTravel(1000);
