...
import { ClaimFormat, CredentialExchangeRecord, CredentialRole, CredentialState, ProofExchangeRecord, ProofRole, ProofState, } from '@credo-ts/core';
...
import { anonCredsCredentialsForProofRequest, difPexCredentialsForRequest, difPexCredentialsForRequest2, testPresentationDefinition1, testW3cCredentialRecord, } from './fixtures/w3c-proof-request';
...
describe('displays a proof request screen', () => {
    ...
    describe('ProofRequest Screen, W3C', () => {
        ...
        test('displays a proof request with all claims available', async () => {
            ...
            const { getByText, getByTestId, queryByText } = render(<BasicAppContext>
          <ProofRequest navigation={useNavigation()} proofId={testProofRequest.id}/>
        </BasicAppContext>);
            await waitFor(() => {
                Promise.resolve();
