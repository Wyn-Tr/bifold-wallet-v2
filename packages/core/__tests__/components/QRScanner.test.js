...
describe('QRScanner Component', () => {
    ...
    test('Scanner with no tabs renders correctly', async () => {
        ...
        <QRScanner showTabs={false} defaultToConnect={false} handleCodeScan={() => Promise.resolve()} navigation={navigation} route={{}}/>
      </BasicAppContext>);
        await act(() => {
            jest.runAllTimers();
        });
    ...
    test('Tap on focus area renders animation', async () => {
        const { getByTestId, queryByTestId } = render(<BasicAppContext>
        <QRScanner showTabs={false} defaultToConnect={false} handleCodeScan={() => Promise.resolve()} navigation={navigation} route={{}}/>
      </BasicAppContext>);
        await act(() => {
            jest.runAllTimers();
    ...
    test('Renders correctly on first tab', async () => {
        const tree = render(<BasicAppContext>
        <QRScanner showTabs={true} defaultToConnect={false} handleCodeScan={() => Promise.resolve()} navigation={navigation} route={{}}/>
      </BasicAppContext>);
        await act(() => {
            jest.runAllTimers();
        });
    ...
    test('Renders correctly on second tab', async () => {
        ...
        const tree = render(<StoreProvider initialState={{
                ...defaultState,
                preferences: {
        ...
        <BasicAppContext>
          <QRScanner showTabs={true} defaultToConnect={true} handleCodeScan={() => Promise.resolve()} navigation={navigation} route={{}}/>
        </BasicAppContext>
      </StoreProvider>);
        await act(() => {
            jest.runAllTimers();
    ...
    test('Contains test IDs', async () => {
        ...
        const { getByTestId } = render(<StoreProvider initialState={{
                ...defaultState,
                preferences: {
                    ...defaultState.preferences,
                    walletName: 'My Wallet - 1234',
