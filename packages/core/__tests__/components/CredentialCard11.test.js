...
describe('CredentialCard11 component', () => {
    ...
    describe('In proof form', () => {
        test('With existing credential and alt credentials', async () => {
            ...
            const { findByTestId } = render(<BasicAppContext>
          <CredentialCard11 credential={credentialRecord} credDefId={'cred_def_id'} proof credentialErrors={[]} handleAltCredChange={handleAltCredChange} hasAltCredentials/>
        </BasicAppContext>);
        ...
        test('Missing credential with help action (cred def ID)', async () => {
            ...
            const { findByTestId } = render(<BasicAppContext>
          <CredentialCard11 proof credDefId={'XUxBrVSALWHLeycAUhrNr9:3:CL:26293:Student Card'} credentialErrors={[CredentialErrors.NotInWallet]}/>
        </BasicAppContext>);
        ...
        test('Missing credential with help action (schema ID)', async () => {
            Linking.openURL = jest.fn();
            const { findByTestId } = render(<BasicAppContext>
          <CredentialCard11 proof schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[CredentialErrors.NotInWallet]}/>
        </BasicAppContext>);
            const getThisCredentialButton = await findByTestId(testIdWithKey('GetThisCredential'));
            expect(getThisCredentialButton).toBeTruthy();
            fireEvent(getThisCredentialButton, 'press');
        ...
        test('Credential is Revoked', async () => {
            const { findByTestId } = render(<BasicAppContext>
          <CredentialCard11 proof={true} schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[CredentialErrors.Revoked]}/>
        </BasicAppContext>);
        ...
        test('Credential in proof is valid', async () => {
            ...
            const { findAllByTestId } = render(<BasicAppContext>
          <CredentialCard11 credential={credentialRecord} displayItems={attributes} proof={true} schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[]}/>
        </BasicAppContext>);
        ...
        test('Credential in proof is missing attribute', async () => {
            ...
            const { findByTestId, queryByText } = render(<BasicAppContext>
          <CredentialCard11 credential={credentialRecord} displayItems={attributes} proof={true} schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[]}/>
        </BasicAppContext>);
        ...
        test('Credential in proof is missing from users wallet', async () => {
            ...
            const { findByTestId, queryByText, findAllByTestId, queryByTestId, queryAllByTestId } = render(<BasicAppContext>
          <CredentialCard11 credential={credentialRecord} displayItems={attributes} proof={true} schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[CredentialErrors.NotInWallet]}/>
        </BasicAppContext>);
        ...
        test('Credential in proof has a predicate error', async () => {
            ...
            const { queryByText, queryByTestId, queryAllByTestId } = render(<BasicAppContext>
          <CredentialCard11 credential={credentialRecord} displayItems={attributes} proof={true} schemaId={'XUxBrVSALWHLeycAUhrNr9:2:student_card:1.0'} credentialErrors={[]}/>
        </BasicAppContext>);
            await waitFor(() => {
                timeTravel(1000);
