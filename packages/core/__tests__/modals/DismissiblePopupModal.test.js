...
import DismissiblePopupModal from '../../src/components/modals/DismissiblePopupModal';
...
describe('DismissiblePopupModal Component', () => {
    test('Renders correctly without call to action', () => {
        ...
        const tree = render(<DismissiblePopupModal title={title} description={description} onDismissPressed={onDismissPressed}/>);
        expect(tree).toMatchSnapshot();
    });
    test('Renders correctly with call to action', () => {
        const onDismissPressed = jest.fn();
        const onCallToActionPressed = jest.fn();
        const tree = render(<DismissiblePopupModal title={title} description={description} onDismissPressed={onDismissPressed} onCallToActionPressed={onCallToActionPressed}/>);
        expect(tree).toMatchSnapshot();
    });
    test('Triggers call to action on press', () => {
        const onDismissPressed = jest.fn();
        const onCallToActionPressed = jest.fn();
        const { getByTestId } = render(<DismissiblePopupModal title={title} description={description} onDismissPressed={onDismissPressed} onCallToActionPressed={onCallToActionPressed}/>);
