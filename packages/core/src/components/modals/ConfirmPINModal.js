...
export var ConfirmPINModalUsage;
(function (ConfirmPINModalUsage) {
...
})(ConfirmPINModalUsage || (ConfirmPINModalUsage = {}));
const ConfirmPINModal = ({ errorModalState, modalUsage = ConfirmPINModalUsage.PIN_CREATE, onBackPressed = () => { }, onConfirmPIN = () => { }, PINOne = '', setPINTwo = () => { }, title = '', visible = false, isLoading = false, }) => {
    ...
    return (<SafeAreaModal style={{ backgroundColor: ColorPalette.brand.primaryBackground }} visible={visible} transparent={false} animationType={'none'} presentationStyle={'fullScreen'} statusBarTranslucent={true}>
      ...
      <ScreenWrapper keyboardActive>
        <View style={style.container}>
          {modalUsage === ConfirmPINModalUsage.PIN_CREATE && (<PINScreenTitleText header={t('PINCreate.Header')} subheader={t('PINCreate.Subheader')}/>)}
          <PINHeader />
          <PINInput label={t('PINCreateConfirm.PINInputLabel')} onPINChanged={async (userPinInput) => {
            setPINTwo(userPinInput);
            setErrorMessage(undefined);
            if (userPinInput.length === PINOne.length) {
                Keyboard.dismiss();
                if (!comparePINEntries(PINOne, userPinInput))
                    setErrorMessage(customErrorMessage);
                else
                    await onConfirmPIN(userPinInput);
            }
        }} testID={testIdWithKey('EnterPIN')} accessibilityLabel={t('PINCreate.EnterPIN')} autoFocus={false} inlineMessage={errorMessage}/>
          {isLoading && (<View style={style.loadingContainer}>
              <LoadingSpinner size={50} color={ColorPalette.brand.primary}/>
            </View>)}
          {errorModalState?.visible && (<AlertModal title={errorModalState?.title} message={errorModalState?.message} submit={errorModalState?.onModalDismiss}/>)}
        </View>
      </ScreenWrapper>
    </SafeAreaModal>);
};
