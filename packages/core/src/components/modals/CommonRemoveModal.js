...
const Dropdown = ({ title, content }) => {
    ...
    return (<>
      <TouchableOpacity onPress={() => setIsCollapsed(!isCollapsed)} accessibilityLabel={title} testID={testIdWithKey(testIdForAccessabilityLabel(title))} style={[
            {
                padding: 15,
      ...
      <Collapsible collapsed={isCollapsed} enablePointerEvents={true}>
        <View style={{ marginTop: 10, borderLeftWidth: 2, borderLeftColor: ColorPalette.brand.modalSecondaryBackground }}>
          <UnorderedList unorderedListItems={content}/>
        </View>
      </Collapsible>
    </>);
};
const CommonRemoveModal = ({ usage, visible, onSubmit, onCancel, extraDetails }) => {
    if (!usage) {
    ...
    const headerImageForType = () => {
        ...
        {usage === ModalUsage.CustomNotificationDecline && (<Assets.svg.deleteNotification style={{ marginBottom: 15 }} {...imageDisplayOptions}/>)}
      </View>);
    };
    const contentForType = () => {
        switch (usage) {
            ...
            <View style={{ marginTop: 25 }}>
              <Dropdown title={t('CredentialDetails.YouWillNotLose')} content={[
                        t('CredentialDetails.YouWillNotLoseListItem1'),
                        t('CredentialDetails.YouWillNotLoseListItem2'),
                    ]}/>
            </View>
            <View style={{ marginTop: 25 }}>
              <Dropdown title={t('CredentialDetails.HowToGetThisCredentialBack')} content={[t('CredentialDetails.HowToGetThisCredentialBackListItem1')]}/>
            </View>
          </View>);
            case ModalUsage.ContactRemoveWithCredentials:
                return (<View style={{ marginBottom: 25 }}>
    ...
    return (<SafeAreaModal transparent={true} visible={visible} animationType="slide">
      <View style={styles.overlay}>
        <SafeAreaView edges={['left', 'right', 'bottom']} style={styles.safeAreaView}>
          <View style={styles.headerView}>
            <TouchableOpacity accessibilityLabel={t('Global.Close')} accessibilityRole={'button'} testID={testIdWithKey('Close')} onPress={() => onCancel && onCancel()} hitSlop={hitSlop}>
          ...
          <View style={styles.controlsContainer}>
            ...
            <View style={{ paddingTop: 10 }}>
              <Button title={titleForConfirmButton()} accessibilityLabel={labelForConfirmButton()} testID={testIdForConfirmButton()} onPress={onSubmit} buttonType={usage === ModalUsage.ContactRemoveWithCredentials ? ButtonType.ModalPrimary : ButtonType.ModalCritical}/>
            </View>
            <View style={{ paddingTop: 10 }}>
              <Button title={t('Global.Cancel')} accessibilityLabel={t('Global.Cancel')} testID={testIdForCancelButton()} onPress={onCancel} buttonType={ButtonType.ModalSecondary}/>
            </View>
          </View>
        </SafeAreaView>
      </View>
