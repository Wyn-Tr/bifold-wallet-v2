...
const ContactListItem = ({ contact, navigation }) => {
    ...
    return (<TouchableOpacity onPress={navigateToContact} testID={testIdWithKey('Contact')} accessibilityLabel={contactLabel} accessibilityRole="button">
      <View style={styles.container}>
        <View style={styles.avatarContainer}>
          {contact.imageUrl ? (<View>
              <Image style={styles.avatarImage} source={{ uri: contact.imageUrl }}/>
            </View>) : (<ThemedText allowFontScaling={false} variant="headingFour" style={styles.avatarPlaceholder}>
              {contactLabelAbbr}
            </ThemedText>)}
        </View>
        <View style={{ flex: 1 }}>
          ...
          <View>
            {message && !hasOnlyInitialMessage && (<ThemedText numberOfLines={1} ellipsizeMode={'tail'}>
                {message.text}
              </ThemedText>)}
          </View>
        </View>
