...
import { BasicMessageRecord, BasicMessageRepository, CredentialExchangeRecord, ProofExchangeRecord, ProofState, V2RequestPresentationMessage, } from '@credo-ts/core';
...
const NotificationListItem = ({ notificationType, notification, customNotification, }) => {
    ...
    const connectionId = notification instanceof BasicMessageRecord ||
    ...
    const isReceivedProof = useMemo(() => {
        return (notificationType === NotificationType.ProofRequest &&
            (notification.state === ProofState.Done ||
                notification.state === ProofState.PresentationSent));
    ...
    const declineCustomNotification = useCallback(async () => {
        customNotification?.onCloseAction(dispatch);
        toggleDeclineModalVisible();
    ...
    const commonRemoveModal = () => {
        ...
        return usage !== undefined && onSubmit !== undefined ? (<CommonRemoveModal usage={usage} visible={declineModalVisible} onSubmit={onSubmit} onCancel={toggleDeclineModalVisible}/>) : null;
    };
    useEffect(() => {
            ...
            switch (notificationType) {
                ...
                case NotificationType.ProofRequest: {
                    ...
                    if (message instanceof V2RequestPresentationMessage) {
                        ...
                        const name = attachment?.getDataAsJson()?.name ?? null;
                        body = name ?? body;
                    }
                    details = {
                        type: InfoBoxType.Info,
        ...
        if (notificationType === NotificationType.Custom && customNotification) {
            setDetails({
                type: InfoBoxType.Info,
                title: t(customNotification?.title),
                body: t(customNotification?.description),
                buttonTitle: t(customNotification?.buttonTitle),
            });
        }
        else {
            getDetails();
    ...
    useEffect(() => {
        ...
        switch (notificationType) {
            ...
            case NotificationType.ProofRequest:
                if (notification.state === ProofState.Done ||
                    notification.state === ProofState.PresentationReceived) {
                    onPress = () => {
                        navigation.getParent()?.navigate(Stacks.ContactStack, {
                    ...
                    onPress = () => {
                        navigation.getParent()?.navigate(Stacks.ConnectionStack, {
                            screen: Screens.Connection,
                            params: { proofId: notification.id },
                        });
                    };
                }
                onClose = toggleDeclineModalVisible;
            ...
            case NotificationType.Proof:
                onPress = () => navigation.getParent()?.navigate(Stacks.NotificationStack, {
                    screen: Screens.ProofDetails,
                    params: { recordId: notification.id, isHistory: true },
                });
                break;
            ...
            case NotificationType.Custom:
                onPress = () => {
                    if (customNotification?.type === OpenIDCustomNotificationType.CredentialExpired &&
    ...
    return (<View style={[styles.container, styleConfig.containerStyle]} testID={testIdWithKey('NotificationListItem')}>
      <View style={styles.headerContainer}>
        <View style={styles.icon}>
          <Icon accessible={false} name={styleConfig.iconName} size={iconSize} color={styleConfig.iconColor}/>
        </View>
        <ThemedText variant="bold" style={[styles.headerText, styleConfig.textStyle]} testID={testIdWithKey('HeaderText')}>
          {details.title}
        </ThemedText>
        {[
        ...
        ].includes(notificationType) && (<View>
            <TouchableOpacity accessibilityLabel={t('Global.Dismiss')} accessibilityRole={'button'} testID={testIdWithKey(`Dismiss${notificationType}`)} onPress={closeAction} hitSlop={hitSlop}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <Button title={details.buttonTitle ?? t('Global.View')} accessibilityLabel={details.buttonTitle ?? t('Global.View')} testID={testIdWithKey(`View${notificationType}${isReceivedProof ? 'Received' : ''}`)} buttonType={ButtonType.Primary} onPress={action}/>
      </View>
      {commonRemoveModal()}
    </View>);
};
