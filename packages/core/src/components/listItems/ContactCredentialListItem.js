...
const ContactCredentialListItem = ({ credential, onPress }) => {
    ...
    const { overlay } = useBranding(params);
    ...
    return (<TouchableOpacity onPress={onPress} style={styles.container} accessibilityHint={t('ContactDetails.GoToCredentialDetail')} accessibilityLabel={`${t('ContactDetails.CredentialName')}: ${overlay?.metaOverlay?.name}`} accessibilityRole={'button'}>
