...
const PINHeader = ({ updatePin }) => {
    const { TextTheme } = useTheme();
    const { t } = useTranslation();
    const [{ PINScreensConfig }] = useServices([TOKENS.CONFIG]);
    return PINScreensConfig.useNewPINDesign ? (<View style={style.infoBox}>
      <InfoBox title={t('PINCreate.InfoBox.title')} message={t('PINCreate.InfoBox.message')} notificationType={InfoBoxType.Info} renderShowDetails/>
    </View>) : (<View>
