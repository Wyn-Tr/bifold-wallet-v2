...
const ScanCamera = ({ handleCodeScan, error, enableCameraOnError, torchActive }) => {
    ...
    return (<View style={[StyleSheet.absoluteFill, { transform: [{ rotate: orientationDegrees[orientation] ?? '0deg' }] }]}>
      {device && (<>
          <Camera ref={camera} style={StyleSheet.absoluteFillObject} device={device} torch={torchActive ? 'on' : 'off'} isActive={cameraActive} codeScanner={codeScanner} format={format}/>
          <Pressable testID={testIdWithKey('ScanCameraTapArea')} style={StyleSheet.absoluteFill} onPressIn={(e) => {
                handleFocusTap(e);
            }}/>
          {focusPoint && (<Animated.View testID={testIdWithKey('FocusIndicator')} style={[
