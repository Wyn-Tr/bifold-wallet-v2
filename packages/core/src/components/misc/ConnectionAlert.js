...
const ConnectionAlert = ({ connectionLabel }) => {
    ...
    return (<View style={styles.notifyTextContainer}>
      <View style={styles.row}>
        <ThemedText variant="title" style={styles.notifyTitle}>
          {t('ConnectionAlert.AddedContacts')}
        </ThemedText>
        <TouchableOpacity testID={t('Global.Info')} accessibilityLabel={t('ConnectionAlert.WhatAreContacts')} accessibilityRole={'button'} onPress={toggleInfoCard} hitSlop={hitSlop}>
          <Icon name={'information-outline'} size={30} style={styles.informationIcon}/>
        </TouchableOpacity>
      </View>
      {infoCardVisible && (<PopupModal notificationType={InfoBoxType.Info} title={t('ConnectionAlert.WhatAreContacts')} bodyContent={<View>
              <ThemedText variant="popupModalText">{t('ConnectionAlert.PopupIntro')}</ThemedText>
              <UnorderedList unorderedListItems={[
              ...
              <Link style={{ marginBottom: 8 }} onPress={navigateToSettings} linkText={t('ConnectionAlert.SettingsLink')}/>
              <ThemedText variant="popupModalText">{t('ConnectionAlert.PrivacyMessage')}</ThemedText>
            </View>} onCallToActionLabel={t('ConnectionAlert.PopupExit')} onCallToActionPressed={toggleInfoCard}/>)}
