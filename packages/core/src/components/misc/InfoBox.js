...
const InfoBox = ({ notificationType, title, description, bodyContent, message, callToActionDisabled, callToActionIcon, secondaryCallToActionTitle, secondaryCallToActionPressed, secondaryCallToActionDisabled, secondaryCallToActionIcon, onCallToActionPressed, onCallToActionLabel, onClosePressed, showVersionFooter, renderShowDetails = false, }) => {
    ...
    return (<View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        {onClosePressed && (<View>
            <TouchableOpacity accessibilityLabel={t('Global.Dismiss')} accessibilityRole={'button'} testID={testIdWithKey(`Dismiss${notificationType}`)} onPress={onClosePressed} hitSlop={hitSlop}>
      ...
      <ScrollView style={styles.bodyContainer}>
        <>
          ...
          {(description || (message && showDetails)) && (<ThemedText style={styles.bodyText} testID={testIdWithKey('BodyText')}>
              {showDetails ? message : description}
            </ThemedText>)}
          {message && !showDetails && (showDetailsInfo ?? true) && (<TouchableOpacity accessibilityLabel={t('Global.ShowDetails')} accessibilityRole="button" testID={testIdWithKey('ShowDetails')} style={{ marginVertical: 14 }} onPress={onShowDetailsTouched} hitSlop={hitSlop}>
          ...
          {onCallToActionPressed && (<View style={{ paddingTop: 10 }}>
              <Button title={onCallToActionLabel || t('Global.Okay')} accessibilityLabel={onCallToActionLabel || t('Global.Okay')} testID={onCallToActionLabel ? testIdWithKey(onCallToActionLabel) : testIdWithKey('Okay')} buttonType={ButtonType.Primary} onPress={onCallToActionPressed} disabled={callToActionDisabled}>
                {callToActionIcon}
              </Button>
            </View>)}
          {secondaryCallToActionTitle && secondaryCallToActionPressed && (<View style={{ paddingTop: 10 }}>
              <Button title={secondaryCallToActionTitle} accessibilityLabel={secondaryCallToActionTitle} testID={testIdWithKey(secondaryCallToActionTitle)} buttonType={ButtonType.Secondary} onPress={secondaryCallToActionPressed} disabled={secondaryCallToActionDisabled}>
                {secondaryCallToActionIcon}
              </Button>
            </View>)}
          {showVersionFooter ? (<ThemedText variant="caption" style={{ flex: 1, marginTop: 8, textAlign: 'center' }} testID={testIdWithKey('VersionNumber')}>
