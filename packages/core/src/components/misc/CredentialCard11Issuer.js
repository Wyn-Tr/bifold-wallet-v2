...
const CredentialIssuerBody = ({ overlay, overlayType, hasBody, proof }) => {
    ...
    if (!hasBody)
        return;
    return (<>
      {isBranding10 ? (<View style={{ flexDirection: 'row' }}>
          <ThemedText variant="label" testID={testIdWithKey('CredentialIssuer')} style={[
        ...
        </View>) : (<View style={styles.credentialIssuerContainer}>
          <View style={{
                flexDirection: 'row',
                alignSelf: 'flex-end',
            }}>
            <ThemedText testID={testIdWithKey('CredentialIssuer')} style={[
                styles.textContainer,
                {
                    fontWeight: '500',
                    fontSize: 12,
