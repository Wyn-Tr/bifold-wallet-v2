 ...
 */
const VerifierCredentialCard = ({ style = {}, displayItems = [], elevated = false, credDefId, schemaId, preview, onChangeValue, brandingOverlayType = BrandingOverlayType.Branding10, }) => {
    ...
    const attributeFormats = overlay.bundle?.bundle.attributes
        .map((attr) => {
    ...
    const AttributeItem = ({ item }) => {
        ...
        return (<View>
        <ThemedText variant="labelSubtitle" style={[
        ...
        {value && (<View>
            {isDataUrl(value) ? (<TouchableOpacity accessibilityLabel={t('Record.Zoom')} accessibilityRole="imagebutton" testID={testIdWithKey('zoom')} onPress={() => setShowImageModal(true)}>
                <Image style={styles.imageAttr} source={{ uri: value }}/>
              </TouchableOpacity>) : (<ThemedText variant="bold" style={styles.textContainer} testID={testIdWithKey('AttributeValue')}>
                {value}
              </ThemedText>)}
          </View>)}
        {showImageModal && <ImageModal uri={value} onDismissPressed={() => setShowImageModal(false)}/>}
      </View>);
    };
    const PredicateItem = ({ item }) => {
        ...
        return (<View>
        <ThemedText variant="labelSubtitle" style={[styles.textContainer, predicateStyles.predicateLabel]} testID={testIdWithKey('PredicateName')}>
          {ylabel}
        </ThemedText>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          {item.satisfied && !preview ? (<Icon style={{ marginRight: 5 }} size={24} name={'check-circle'} color={ColorPalette.semantic.success}/>) : null}
          ...
          {item.parameterizable && preview && onChangeValue ? (<TextInput keyboardType="numeric" style={[TextTheme.bold, predicateStyles.input]} onChangeText={(value) => setCurrentValue(value)} onBlur={() => {
                    onChangeValue(schemaId, item.label || item.name || '', item.name || '', currentValue);
                }} testID={testIdWithKey('PredicateInput')}>
              {currentValue}
            </TextInput>) : (<ThemedText variant="bold" style={styles.textContainer} testID={testIdWithKey('PredicateValue')}>
              {item.pValue}
            </ThemedText>)}
        </View>
      </View>);
    };
    const CardAttribute = ({ item }) => {
        ...
        {item instanceof Attribute && <AttributeItem item={item}/>}
        {item instanceof Predicate && <PredicateItem item={parsedPredicate}/>}
      </View>);
    };
    ...
    const CredentialCardPrimaryBody = () => {
        ...
        <View>
          {isBranding10 && (<CredentialIssuerBody overlay={overlay} overlayType={brandingOverlayType} hasBody={overlay.metaOverlay?.issuer !== 'Unknown Contact'} proof/>)}
          <View style={styles.primaryBodyNameContainer}>
            <ThemedText variant="bold" testID={testIdWithKey('CredentialName')} style={[styles.textContainer, styles.credentialName]}>
              {overlay.metaOverlay?.name}
            </ThemedText>
          </View>
        </View>
        <FlatList data={displayItems} scrollEnabled={false} initialNumToRender={displayItems?.length} keyExtractor={({ name }) => name} renderItem={({ item }) => {
                return renderCardAttribute(item);
            }}/>
        {brandingOverlayType === BrandingOverlayType.Branding11 && (<CredentialIssuerBody overlay={overlay} overlayType={brandingOverlayType} hasBody={overlay.metaOverlay?.issuer !== 'Unknown Contact'}/>)}
      </View>);
    };
    ...
    const CredentialCard = () => {
        ...
        const accessibilityLabel = isBranding11
            ? `${watermarkLabel} ${overlay.metaOverlay?.name ?? ''} ${t('Credentials.Credential')}${displayItems.length > 0 ? ',' : ''}` +
            ...
            : `${issuerAccessibilityLabel}, ${overlay.metaOverlay?.watermark ?? ''} ${overlay.metaOverlay?.name ?? ''} ${t('Credentials.Credential')}.` +
                displayItems.map((item) => {
                    const { label, value } = parseAttribute(item);
                    if (label) {
        ...
        return (<View style={styles.cardContainer} accessible={true} accessibilityLabel={accessibilityLabel}>
        <CredentialCardSecondaryBody />
        <CredentialCard11Logo noLogoText={overlay.metaOverlay?.name ?? overlay.metaOverlay?.issuer ?? 'C'} overlay={overlay} elevated={elevated}/>
    ...
    return overlay.bundle ? (<View style={[
      ...
      <View testID={testIdWithKey('CredentialCard')} style={{ overflow: 'hidden' }}>
        {overlay.metaOverlay?.watermark && (<CardWatermark width={dimensions.cardWidth} height={dimensions.cardHeight} style={{ color: getCardWatermarkTextColor(overlay.brandingOverlay?.primaryBackgroundColor) }} watermark={overlay.metaOverlay?.watermark}/>)}
        <CredentialCard />
      </View>
    </View>) : null;
};
