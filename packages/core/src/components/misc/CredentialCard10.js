...
import { Image, ImageBackground, StyleSheet, Text, View, useWindowDimensions, TouchableOpacity, } from 'react-native';
import { TOKENS, useServices } from '../../container-api';
import { useTheme } from '../../contexts/theme';
import { credentialTextColor, getCredentialIdentifiers, isValidAnonCredsCredential, toImageSource, getEffectiveCredentialName, } from '../../utils/credential';
 ...
 */
const CredentialCard10 = ({ credential, style = {}, onPress = undefined }) => {
    ...
    const CredentialCardHeader = () => {
        return (<View style={styles.outerHeaderContainer}>
        <View testID={testIdWithKey('CredentialCardHeader')} style={styles.innerHeaderContainer}>
          {overlay?.brandingOverlay?.header?.imageSource && (<Image source={toImageSource(overlay?.brandingOverlay?.header?.imageSource)} style={{
                    flex: !overlay?.brandingOverlay?.header?.hideIssuer ? 1 : 4,
                    resizeMode: 'contain',
                    maxHeight: styles.outerHeaderContainer.height - borderPadding,
                }}/>)}
          {overlay?.brandingOverlay?.header?.hideIssuer ? null : (<Text numberOfLines={1} ellipsizeMode="tail" style={[
                    TextTheme.label,
                    {
                        color: overlay?.brandingOverlay?.header?.color ??
                            credentialTextColor(ColorPalette, overlay?.brandingOverlay?.header?.backgroundColor || overlay?.brandingOverlay?.backgroundColor),
                        paddingHorizontal: 0.5 * paddingHorizontal,
                        flex: !overlay?.brandingOverlay?.header?.imageSource ? 4 : 3,
                        textAlignVertical: 'center',
                    },
                ]} testID={testIdWithKey('CredentialIssuer')} maxFontSizeMultiplier={1}>
          ...
          <Text numberOfLines={1} ellipsizeMode="tail" style={[
            ...
            ]} testID={testIdWithKey('CredentialName')} maxFontSizeMultiplier={1}>
    ...
    const CredentialCardFooter = ({ revoked = false }) => {
        return (<View testID={testIdWithKey('CredentialCardFooter')} style={styles.footerContainer}>
        {revoked ? (<View style={styles.revokedFooter}>
            <Text style={[TextTheme.label, { color: ColorPalette.semantic.error }]} testID={testIdWithKey('CredentialRevoked')}>
              {t('CredentialDetails.Revoked')}
            </Text>
          </View>) : (<Text style={[
                    TextTheme.caption,
                    {
                        color: overlay?.brandingOverlay?.footer?.color ??
                            credentialTextColor(ColorPalette, overlay?.brandingOverlay?.footer?.backgroundColor || overlay?.brandingOverlay?.backgroundColor),
                    },
                ]} testID={testIdWithKey('CredentialIssued')} maxFontSizeMultiplier={1}>
            {t('CredentialDetails.Issued')}: {formatTime(credential.createdAt, { shortMonth: true })}
          </Text>)}
      </View>);
    };
    const CredentialCard = ({ revoked = false }) => {
        return (<>
    ...
    return (<TouchableOpacity accessible={true} accessibilityLabel={`${overlay.metaOverlay?.issuer ? `${t('Credentials.IssuedBy')} ${overlay.metaOverlay?.issuer}` : ''}, ${overlay.metaOverlay?.watermark ?? ''} ${overlay.metaOverlay?.name ?? ''} ${t('Credentials.Credential')}.`} disabled={typeof onPress === 'undefined' ? true : false} onPress={onPress} style={[styles.container, style]} testID={testIdWithKey('ShowCredentialDetails')}>
      <View style={[styles.flexGrow, { overflow: 'hidden' }]} testID={testIdWithKey('CredentialCard')}>
        {overlay?.brandingOverlay?.imageSource ? (<ImageBackground source={toImageSource(overlay?.brandingOverlay?.imageSource)} style={styles.flexGrow} imageStyle={{ borderRadius }}>
            {overlay.metaOverlay?.watermark && (<CardWatermark width={width} height={cardHeight} style={styles.watermark} watermark={overlay.metaOverlay?.watermark}/>)}
