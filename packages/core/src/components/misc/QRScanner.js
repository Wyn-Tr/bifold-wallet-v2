...
const QRScanner = ({ showTabs = false, defaultToConnect, handleCodeScan, error, enableCameraOnError, navigation, }) => {
    ...
    useEffect(() => {
        ...
        if (!showTabs)
            return;
        let headerRight = invitation ? (<IconButton buttonLocation={ButtonLocation.Right} accessibilityLabel={t('Global.Share')} testID={testIdWithKey('ShareButton')} onPress={() => {
                Share.share({ message: invitation ?? '' });
            }} icon="share-variant"/>) : undefined;
        let title = t('Scan.MyQRCode');
        if (firstTabActive) {
            headerRight = (<IconButton buttonLocation={ButtonLocation.Right} accessibilityLabel={t('Global.Share')} testID={testIdWithKey('ShareButton')} onPress={() => {
    ...
    const styleForState = ({ pressed }) => [{ opacity: pressed ? 0.2 : 1 }];
    ...
    return (<View style={styles.container}>
      <SafeAreaView edges={['left', 'right']} style={styles.mainSafeArea} testID={testIdWithKey('QRScanner')}>
        ...
        {!showTabs || firstTabActive ? (<>
            <ScanCamera handleCodeScan={handleCodeScan} error={error} enableCameraOnError={enableCameraOnError} torchActive={torchActive}/>
            ...
            <View style={styles.cameraViewContainer}>
              <View style={styles.messageContainer}>
                {error ? (<>
                    ...
                    {showScanErrorButton && (<Pressable onPress={() => setShowErrorDetailsModal(true)} accessibilityLabel={t('Scan.ShowDetails')} accessibilityRole={'button'} testID={testIdWithKey('ShowDetails')} hitSlop={hitSlop}>
              ...
              <View style={styles.bottomButtonsContainer}>
                {showScanButton && (<View style={{ justifyContent: 'center', alignItems: 'center' }}>
                    <Pressable accessibilityLabel={t('Scan.ScanNow')} accessibilityRole={'button'} testID={testIdWithKey('ScanNow')} onPress={toggleShowInfoBox} style={styleForState} hitSlop={hitSlop}>
                ...
                <View style={{ marginHorizontal: 24, height: 24, marginBottom: 60, flexDirection: 'row' }}>
                  {showScanHelp && (<Pressable accessibilityLabel={t('Scan.ScanHelp')} accessibilityRole={'button'} testID={testIdWithKey('ScanHelp')} onPress={() => navigation.navigate(Screens.ScanHelp)} style={styleForState} hitSlop={hitSlop}>
            ...
            <SafeAreaModal visible={showInfoBox} animationType="fade" transparent>
              <View style={{
                ...
                <InfoBox notificationType={InfoBoxType.Info} title={t('Scan.BadQRCode')} description={t('Scan.BadQRCodeDescription')} onCallToActionPressed={toggleShowInfoBox}/>
              </View>
            </SafeAreaModal>
            {showErrorDetailsModal && (<DismissiblePopupModal title={t('Scan.ErrorDetails')} description={error?.details || t('Scan.NoDetails')} onCallToActionLabel={t('Global.Dismiss')} onCallToActionPressed={() => setShowErrorDetailsModal(false)} onDismissPressed={() => setShowErrorDetailsModal(false)}/>)}
          </>) : (<ScrollView>
            <View style={{ alignItems: 'center' }}>
              ...
              <View style={{ paddingHorizontal: 20, flex: 1 }}>
                ...
                justifyContent: 'center',
                  ...
                  <ThemedText variant="headingTwo" testID={testIdWithKey('WalletName')} style={[styles.walletName, { paddingHorizontal: 20 }]}>
                    {store.preferences.walletName}
                  </ThemedText>
                  <IconButton buttonLocation={ButtonLocation.Right} accessibilityLabel={t('NameWallet.EditWalletName')} testID={testIdWithKey('EditWalletName')} onPress={handleEdit} icon={'pencil'} iconTintColor={TextTheme.headingTwo.color}/>
                </View>
                <ThemedText style={styles.secondaryText}>{t('Connection.ShareQR')}</ThemedText>
              </View>
            </View>
          </ScrollView>)}
        {showTabs ? (<View accessible={true} style={styles.tabContainer} accessibilityRole="tablist">
            <ScanTab title={t('Scan.ScanQRCode')} iconName={'crop-free'} onPress={() => setFirstTabActive(true)} active={firstTabActive}/>
            <ScanTab title={t('Scan.MyQRCode')} iconName={'qr-code'} onPress={() => setFirstTabActive(false)} active={!firstTabActive}/>
          </View>) : null}
      </SafeAreaView>
