...
const SharedDataCard = ({ sharedData }) => {
    ...
    return (<View style={{ marginBottom: 15 }}>
      <VerifierCredentialCard displayItems={attributes} style={bundleResolver.getBrandingOverlayType() === BrandingOverlayType.Branding10
            ? { backgroundColor: ColorPalette.brand.secondaryBackground }
            : undefined} credDefId={sharedData.identifiers.cred_def_id} schemaId={sharedData.identifiers.schema_id} elevated brandingOverlayType={bundleResolver.getBrandingOverlayType()}/>
    </View>);
};
const SharedProofData = ({ recordId, onSharedProofDataLoad }) => {
    ...
    return (<View style={styles.container}>
      {loading || !sharedData ? (<View style={styles.loaderContainer}>
          <LoadingIndicator />
        </View>) : (<View>
          {sharedData.map((item) => (<SharedDataCard key={item.identifiers.cred_def_id} sharedData={item}/>))}
        </View>)}
    </View>);
};
