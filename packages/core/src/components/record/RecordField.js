...
export const AttributeValue = ({ field, style, shown }) => {
    ...
    if ((field.encoding == validEncoding && field.format && validFormat.test(field.format) && field.value) ||
    ...
    return (<ThemedText style={style || styles.text} testID={testIdWithKey('AttributeValue')}>
      {shown ? field.value : hiddenFieldValue}
    </ThemedText>);
};
const RecordField = ({ field, hideFieldValue = false, hideBottomBorder = false, shown = !hideFieldValue, onToggleViewPressed = () => undefined, fieldLabel = null, fieldValue = null, }) => {
    ...
    return (<View style={styles.container}>
      <View style={styles.valueContainer}>
        <View style={styles.fieldLabel}>
          {fieldLabel ? (fieldLabel(field)) : (<ThemedText style={ListItems.recordAttributeLabel} testID={testIdWithKey('AttributeName')}>
              {field.label ?? startCase(field.name || '')}
            </ThemedText>)}
        </View>

        <View style={styles.fieldValue}>
          {hideFieldValue ? (<TouchableOpacity accessible={true} accessibilityLabel={shown ? t('Record.Hide') : t('Record.Show')} accessibilityRole="button" testID={testIdWithKey('ShowHide')} activeOpacity={1} onPress={onToggleViewPressed} style={styles.link}>
