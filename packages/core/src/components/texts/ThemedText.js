 ...
 * @returns
 */
export function ThemedText({ variant = 'normal', maxFontSizeMultiplier, style, ...rest }) {
    const { TextTheme, maxFontSizeMultiplier: maxFontSize } = useTheme();
    return (<Text maxFontSizeMultiplier={maxFontSizeMultiplier ?? maxFontSize} style={[TextTheme[variant], style]} {...rest}/>);
}
