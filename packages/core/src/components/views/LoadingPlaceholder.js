...
const LoadingPlaceholder = ({ workflowType, timeoutDurationInMs = 10000, loadingProgressPercent = 0, onCancelTouched, onTimeoutTriggered, testID, }) => {
    ...
    return (<SafeAreaView style={styles.safeAreaView} testID={testID ?? testIdWithKey('LoadingPlaceholder')} edges={['bottom', 'left', 'right']}>
      <ScrollView>
        ...
        <View style={styles.container}>
          ...
          {overtime && (<InfoTextBox type={InfoBoxType.Info} style={{ marginTop: 20 }}>
              <View style={styles.infoTextBoxContainer}>
                <ThemedText variant="title" style={{ fontWeight: 'bold', marginBottom: 10 }} testID={testIdWithKey('SlowLoadTitle')}>
          ...
          {onCancelTouched && (<View style={styles.buttonContainer}>
              <Button title={t('Global.Cancel')} accessibilityLabel={t('Global.Cancel')} testID={testIdWithKey('Cancel')} buttonType={ButtonType.Primary} onPress={onCancelTouched}/>
            </View>)}
        </View>
      </ScrollView>
    </SafeAreaView>);
