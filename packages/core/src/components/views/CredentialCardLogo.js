...
const CredentialCardLogo = ({ overlay, brandingOverlayType = BrandingOverlayType.Branding10, }) => {
    ...
    const textColor = overlay.brandingOverlay?.secondaryBackgroundColor && overlay.brandingOverlay.secondaryBackgroundColor !== ''
    ...
    return (<View style={styles.logoContainer}>
      {overlay.brandingOverlay?.logo ? (<Image source={toImageSource(overlay.brandingOverlay?.logo)} style={{
                resizeMode: 'cover',
                width: logoHeight,
                height: logoHeight,
                borderRadius: 8,
            }}/>) : (<ThemedText variant="title" style={{ fontSize: 0.5 * logoHeight, color: isBranding11 ? textColor : '#000' }} accessible={false}>
          {logoText}
        </ThemedText>)}
    </View>);
};
