...
export const TourOverlay = (props) => {
    ...
    return currentStep !== undefined ? (<View accessibilityViewIsModal style={{ position: 'absolute', top: 0, left: 0, height: windowHeight + 1, width: windowWidth + 1 }} testID={testIdWithKey('SpotlightOverlay')}>
      <Svg testID={testIdWithKey('SpotOverlay')} height={windowHeight + 1} width={windowWidth + 1} viewBox={viewBox} onPress={handleBackdropPress} shouldRasterizeIOS={true} renderToHardwareTextureAndroid={true}>
        <Defs>
        ...
        <Rect height={windowHeight + 1} width={windowWidth + 1} fill={color} mask="url(#mask)" opacity={backdropOpacity}/>
      </Svg>

      <View ref={tooltipRef} testID={testIdWithKey('SpotTooltip')} style={{ ...tooltipStyle, opacity: 1, position: 'absolute' }}>
        <tourStep.Render currentTour={currentTour} currentStep={currentStep} changeSpot={changeSpot} next={next} previous={previous} stop={stop}/>
      </View>
    </View>) : null;
};
