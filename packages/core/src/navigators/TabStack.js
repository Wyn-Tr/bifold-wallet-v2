...
const TabStack = () => {
    ...
    // Injectable stacks
    HomeStack, CredentialStack,] = useServices([
    ...
    const handleDeepLink = useCallback(async (deepLink) => {
        ...
        try {
            await connectFromScanOrDeepLink(deepLink, agent, logger, navigation, true, // isDeepLink
            enableImplicitInvitations, enableReuseConnections);
        }
        catch (err) {
            const error = new BifoldError(t('Error.Title1039'), t('Error.Message1039'), err?.message ?? err, 1039);
    ...
    return (<SafeAreaView style={{ flex: 1, backgroundColor: NavigationTheme.colors.primary }} edges={['left', 'right', 'top']}>
      <Tab.Navigator initialRouteName={TabStacks.HomeStack} screenOptions={{
            unmountOnBlur: true,
        ...
        <Tab.Screen name={TabStacks.HomeStack} component={HomeStack} options={{
            tabBarIconStyle: styles.tabBarIcon,
            tabBarIcon: ({ color, focused }) => (<AttachTourStep tourID={BaseTourID.HomeTour} index={1}>
                <View style={{ ...TabTheme.tabBarContainerStyle, justifyContent: showLabels ? 'flex-end' : 'center' }}>
                  {focused ? (<Assets.svg.tabOneFocusedIcon height={30} width={30} fill={color}/>) : (<Assets.svg.tabOneIcon height={30} width={30} fill={color}/>)}
        ...
        <Tab.Screen name={TabStacks.ConnectStack} options={{
            tabBarIconStyle: styles.tabBarIcon,
                ...
                <AttachTourStep tourID={BaseTourID.HomeTour} index={0} fill>
                  <View style={{
                    ...
                    alignItems: 'center',
                        ...
                        <View accessible={true} accessibilityRole={'button'} accessibilityLabel={t('TabStack.Scan')} style={{ ...TabTheme.focusTabIconStyle }}>
                          <Assets.svg.tabTwoIcon height={30} width={30} fill={TabTheme.tabBarButtonIconStyle.color} style={{ paddingLeft: 0.5, paddingRight: 0.5 }}/>
                        </View>

                        {showLabels && (<ThemedText maxFontSizeMultiplier={1.2} style={{
                        ...TabTheme.tabBarTextStyle,
                        color: focused ? TabTheme.tabBarActiveTintColor : TabTheme.tabBarInactiveTintColor,
                        marginTop: 5,
                    }}>
        ...
        <Tab.Screen name={TabStacks.CredentialStack} component={CredentialStack} options={{
            tabBarIconStyle: styles.tabBarIcon,
            tabBarIcon: ({ color, focused }) => (<AttachTourStep tourID={BaseTourID.HomeTour} index={2}>
                <View style={{ ...TabTheme.tabBarContainerStyle, justifyContent: showLabels ? 'flex-end' : 'center' }}>
                  {focused ? (<Assets.svg.tabThreeFocusedIcon height={30} width={30} fill={color}/>) : (<Assets.svg.tabThreeIcon height={30} width={30} fill={color}/>)}
                  {showLabels && (<Text style={{
                        ...TabTheme.tabBarTextStyle,
                        color: focused ? TabTheme.tabBarActiveTintColor : TabTheme.tabBarInactiveTintColor,
                        fontWeight: focused ? TextTheme.bold.fontWeight : TextTheme.normal.fontWeight,
