...
const PINChange = ({ navigation }) => {
    ...
    const [Button, inlineMessages, logger, historyManagerCurried, historyEnabled, historyEventsLogger, { preventScreenCapture, PINScreensConfig }, PINHeader,] = useServices([
        TOKENS.COMP_BUTTON,
    ...
    const { PINValidations, validatePINEntry, inlineMessageField1, inlineMessageField2, modalState, PINSecurity, clearModal, setModalState, comparePINEntries, setInlineMessageField1, setInlineMessageField2, } = usePINValidation(PIN);
    ...
    const handleConfirmPINModal = async (userPinInput) => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1049'), t('Error.Message1049'), err?.message ?? err, 1049);
    ...
    return (<ScreenWrapper keyboardActive>
      <View style={style.screenContainer}>
        <View style={style.contentContainer}>
          <PINHeader updatePin/>
          {!PINScreensConfig.useNewPINDesign && (<PINInput label={t('PINChange.EnterOldPINTitle')} testID={testIdWithKey('EnterOldPIN')} accessibilityLabel={t('PINChange.EnterOldPIN')} onPINChanged={(p) => {
                setPINOld(p);
            }} onSubmitEditing={handleChangePinTap}/>)}
          <PINInput label={t('PINChange.EnterPINTitle')} onPINChanged={async (userPinInput) => {
        ...
        }} testID={testIdWithKey('EnterPIN')} accessibilityLabel={t('PINCreate.EnterPIN')} autoFocus={false} inlineMessage={inlineMessageField1} onSubmitEditing={handleChangePinTap}/>
          {!PINScreensConfig.useNewPINDesign && (<PINInput label={t('PINCreateConfirm.PINInputLabel')} onPINChanged={async (userPinInput) => {
                setInlineMessageField2(undefined);
                setPINTwo(userPinInput);
            }} testID={testIdWithKey('ConfirmPIN')} accessibilityLabel={t('PINCreateConfirm.PINInputLabel')} autoFocus={false} inlineMessage={inlineMessageField2} onSubmitEditing={handleChangePinTap}/>)}
          {PINSecurity.displayHelper && <PINValidationHelper validations={PINValidations}/>}
          {modalState.visible && (<AlertModal title={modalState.title} message={modalState.message} submit={modalState.onModalDismiss}/>)}
        </View>
        {!PINScreensConfig.useNewPINDesign && (<View style={style.controlsContainer}>
            <Button title={t('PINChange.ChangePIN')} testID={testIdWithKey('ChangePIN')} accessibilityLabel={t('Global.Continue')} buttonType={ButtonType.Primary} disabled={isContinueDisabled} onPress={handleChangePinTap} ref={createPINButtonRef}>
              {isLoading ? <ButtonLoading /> : null}
            </Button>
          </View>)}
      </View>
      <VerifyPINModal onAuthenticationComplete={onAuthenticationComplete} onBackPressed={onVerifyPINModalBackPressed} onCancelAuth={onCancelAuth} PINVerifyModalUsage={PINEntryUsage.ChangePIN} title={t('Screens.EnterPIN')} visible={verifyPINModalVisible}/>
      <ConfirmPINModal errorModalState={modalState} isLoading={isLoading} modalUsage={ConfirmPINModalUsage.PIN_CHANGE} onBackPressed={onConfirmPINModalBackPressed} onConfirmPIN={handleConfirmPIN} PINOne={PIN} setPINTwo={setPINTwo} title={t('Screens.ConfirmPIN')} visible={confirmPINModalVisible}/>
    </ScreenWrapper>);
};
