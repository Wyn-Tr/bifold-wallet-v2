...
const Connection = ({ navigation, route }) => {
    ...
    const [logger, { useNotifications }, { connectionTimerDelay, autoRedirectConnectionToHome, enableChat }, attestationMonitor, historyManagerCurried, historyEnabled, historyEventsLogger,] = useServices([
        TOKENS.UTIL_LOGGER,
    ...
    const merge = (current, next) => ({ ...current, ...next });
    ...
    useEffect(() => {
        ...
        // to Chat
        if (connection && !Object.values(GoalCodes).includes(oobRecord?.outOfBandInvitation.goalCode ?? '')) {
    ...
    useEffect(() => {
        ...
        if (state.notificationRecord.type === 'W3cCredentialRecord' ||
            state.notificationRecord.type === 'SdJwtVcRecord' ||
            state.notificationRecord.type === 'MdocRecord') {
    ...
    useEffect(() => {
        ...
        for (const notification of notifications) {
            ...
            if ((connection && notification.connectionId === connection.id) ||
                oobRecord
                    ?.getTags()
                    ?.invitationRequestsThreadIds?.includes(notification?.threadId ?? '')) {
                logger?.info(`Connection: Handling notification ${notification.id}`);
                dispatch({ notificationRecord: notification });
                break;
            }
            if (notification.type === 'W3cCredentialRecord' ||
                notification.type === 'SdJwtVcRecord' ||
                notification.type === 'MdocRecord' ||
                notification.type === 'OpenId4VPRequestRecord') {
                dispatch({ notificationRecord: notification });
                break;
            }
        }
    ...
    const displayComponent = () => {
        if (state.inProgress || state.attestationLoading) {
            return (<LoadingPlaceholder workflowType={loadingPlaceholderWorkflowType()} timeoutDurationInMs={connTimerDelay} loadingProgressPercent={state.percentComplete} onCancelTouched={onDismissModalTouched} onTimeoutTriggered={autoRedirectConnectionToHome ? onDismissModalTouched : undefined} testID={testIdWithKey('ConnectionLoading')}/>);
        }
        if (state.shouldShowProofComponent) {
