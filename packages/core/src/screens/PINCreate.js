...
import { AccessibilityInfo, DeviceEventEmitter, Keyboard, StyleSheet, View, findNodeHandle, } from 'react-native';
...
const PINCreate = ({ explainedStatus, setAuthenticated }) => {
    ...
    const [PINExplainer, PINHeader, { showPINExplainer, preventScreenCapture, PINScreensConfig }, Button, inlineMessages,] = useServices([
    ...
    const { PINValidations, validatePINEntry, inlineMessageField1, inlineMessageField2, modalState, PINSecurity } = usePINValidation(PIN);
    ...
    const passcodeCreate = useCallback(async (PIN) => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1040'), t('Error.Message1040'), err?.message ?? err, 1040);
    ...
    return explained ? (<ScreenWrapper keyboardActive>
      <View style={style.screenContainer}>
        ...
        }} testID={testIdWithKey('EnterPIN')} accessibilityLabel={t('PINCreate.EnterPIN')} autoFocus={false} inlineMessage={inlineMessageField1}/>
          {!PINScreensConfig.useNewPINDesign && (<PINInput label={t('PINCreate.ReenterPIN')} onPINChanged={(userPinInput) => {
            ...
            }} testID={testIdWithKey('ReenterPIN')} accessibilityLabel={t('PINCreate.ReenterPIN')} autoFocus={false} ref={PINTwoInputRef} inlineMessage={inlineMessageField2}/>)}
        ...
        {!PINScreensConfig.useNewPINDesign && (<View style={style.controlsContainer}>
            <Button title={t('PINCreate.CreatePIN')} testID={testIdWithKey('CreatePIN')} accessibilityLabel={t('PINCreate.CreatePIN')} buttonType={ButtonType.Primary} disabled={isContinueDisabled} onPress={onCreatePIN} ref={createPINButtonRef}>
              {isLoading ? <ButtonLoading /> : null}
            </Button>
          </View>)}
        <ConfirmPINModal modalUsage={ConfirmPINModalUsage.PIN_CREATE} onBackPressed={modalBackButtonPressed} onConfirmPIN={onConfirmPIN} PINOne={PIN} setPINTwo={setPINTwo} title={t('Screens.CreatePIN')} visible={PINConfirmModalVisible} isLoading={isLoading}/>
      </View>
    </ScreenWrapper>) : (<PINExplainer continueCreatePIN={continueCreatePIN}/>);
};
