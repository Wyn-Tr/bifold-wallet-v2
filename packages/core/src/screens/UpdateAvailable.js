...
const UpdateAvailable = ({ appleAppStoreUrl, googlePlayStoreUrl }) => {
    ...
    return (<SafeAreaView style={{ flex: 1 }} edges={['bottom', 'left', 'right']}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {hasStoreLinks && (<Link style={{ marginVertical: 24 }} onPress={onPressWhatIsNew} linkText={t('AppUpdate.LearnMore')}/>)}

        <View style={styles.buttonContainer}>
          {hasStoreLinks && (<View style={{ marginBottom: 15 }}>
              <Button title={t('AppUpdate.UpdateNow')} accessibilityLabel={t('AppUpdate.UpdateNow')} testID={testIdWithKey('UpdateNow')} buttonType={ButtonType.Primary} onPress={onPressWhatIsNew}/>
            </View>)}
          <Button title={t('AppUpdate.UpdateLater')} accessibilityLabel={t('AppUpdate.UpdateLater')} testID={testIdWithKey('UpdateLater')} buttonType={ButtonType.Secondary} onPress={onPressLater}/>
        </View>
      </ScrollView>
    </SafeAreaView>);
};
