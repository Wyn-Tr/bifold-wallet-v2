...
const PINExplainer = ({ continueCreatePIN }) => {
    ...
    return (<SafeAreaView style={style.safeAreaView} edges={['bottom', 'left', 'right']}>
      <ScrollView contentContainerStyle={style.scrollViewContentContainer}>
        <View>
          ...
          <ThemedText style={{ marginTop: 30, marginBottom: 30 }}>
            <Trans i18nKey="PINCreate.Explainer.PINReminder" components={{
      ...
      <View style={style.footer}>
        <Button title={t('Global.Continue')} accessibilityLabel={t('Global.Continue')} testID={testIdWithKey('ContinueCreatePIN')} onPress={continueCreatePIN} buttonType={ButtonType.Primary}/>
      </View>
    </SafeAreaView>);
};
