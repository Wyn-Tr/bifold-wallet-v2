...
import { AppState, DeviceEventEmitter, InteractionManager, Keyboard, Pressable, StyleSheet, Vibration, View, } from 'react-native';
...
const PINEnter = ({ setAuthenticated }) => {
    ...
    const [logger, { preventScreenCapture, enableHiddenDevModeTrigger, attemptLockoutConfig: { thresholdRules } = attemptLockoutConfig, PINScreensConfig, }, inlineMessages,] = useServices([TOKENS.UTIL_LOGGER, TOKENS.CONFIG, TOKENS.INLINE_ERRORS]);
    ...
    const unlockWalletWithPIN = useCallback(async (PIN) => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1041'), t('Error.Message1041'), err?.message ?? err, 1041);
    ...
    const HelpText = useMemo(() => {
        ...
        return (<>
        {PINScreensConfig.useNewPINDesign && store.lockout.displayNotification && (<ThemedText variant={'headingTwo'} style={style.helpText}>
            {t('PINEnter.Title')}
          </ThemedText>)}
        <ThemedText variant={showHelpText ? 'normal' : 'headingThree'} style={PINScreensConfig.useNewPINDesign ? style.helpText : style.helpTextSubtitle}>
          {header}
        </ThemedText>
        <ThemedText variant={PINScreensConfig.useNewPINDesign ? 'normal' : 'labelSubtitle'} style={PINScreensConfig.useNewPINDesign ? style.helpTextSubtitle : style.helpText}>
          {subheader}
        </ThemedText>
      </>);
    }, [
    ...
    return (<ThemedBackground screenId="onboarding" style={{ flex: 1 }}>
      <ScreenWrapper keyboardActive>
        <View style={style.screenContainer}>
        <View>
          {PINScreensConfig.useNewPINDesign && (<ThemedText variant="bold" style={style.appTitle}>
              {t('PINEnter.AppTitle')}
            </ThemedText>)}
          <Pressable onPress={enableHiddenDevModeTrigger ? incrementDeveloperMenuCounter : () => { }} testID={testIdWithKey('DeveloperCounter')}>
            {HelpText}
          </Pressable>
          {!PINScreensConfig.useNewPINDesign && (<ThemedText variant="bold" style={style.inputLabel}>
              {t('PINEnter.EnterPIN')}
            </ThemedText>)}
          <PINInput onPINChanged={(userPinInput) => {
            setPIN(userPinInput);
            if (userPinInput.length === minPINLength) {
                Keyboard.dismiss();
                onPINInputCompleted(userPinInput);
            }
        }} testID={testIdWithKey('EnterPIN')} accessibilityLabel={t('PINEnter.EnterPIN')} autoFocus={true} inlineMessage={inlineMessageField} onSubmitEditing={(userPinInput) => {
            onPINInputCompleted(userPinInput);
        }}/>
          {!PINScreensConfig.useNewPINDesign && (<ThemedText variant="bold" style={style.forgotPINText} onPress={() => setForgotPINModalVisible(true)} testID={testIdWithKey('ForgotPINLink')} accessible={true} accessibilityRole="link" accessibilityLabel={t('PINEnter.ForgotPINLink')}>
              {t('PINEnter.ForgotPINLink')}
            </ThemedText>)}
          {showForgotPINMessage && PINScreensConfig.useNewPINDesign && (<ThemedText variant="normal" style={style.forgotPINText} testID={testIdWithKey('ForgotPINDescription')} accessible={true} accessibilityRole="text" accessibilityLabel={t('PINEnter.ForgotPINModalDescription')}>
              {t('PINEnter.ForgotPINModalDescription')}
            </ThemedText>)}
        </View>
        {PINScreensConfig.useNewPINDesign && !continueEnabled && (<View style={style.loadingContainer}>
            <LoadingSpinner size={50} color={ColorPalette.brand.primary}/>
            <ThemedText variant="normal">{t('PINEnter.Loading')}</ThemedText>
          </View>)}
        <View>
          {!PINScreensConfig.useNewPINDesign && (<View style={style.buttonContainer}>
              <Button title={t('PINEnter.Unlock')} buttonType={ButtonType.Primary} testID={testIdWithKey('Enter')} disabled={isContinueDisabled} accessibilityLabel={t('PINEnter.Unlock')} onPress={() => {
                Keyboard.dismiss();
          ...
          {store.preferences.useBiometry && (<>
              ...
              <View style={style.biometricsButtonContainer}>
                <Button title={t('PINEnter.BiometricsUnlock')} buttonType={ButtonType.Secondary} testID={testIdWithKey('BiometricsUnlock')} disabled={!continueEnabled} accessibilityLabel={t('PINEnter.BiometricsUnlock')} onPress={loadWalletCredentials}/>
              </View>
            </>)}
      ...
      {alertModalVisible ? (<PopupModal notificationType={InfoBoxType.Info} title={t('PINEnter.IncorrectPIN')} bodyContent={<>
              <ThemedText variant="popupModalText" style={style.modalText}>
                {alertModalMessage}
              </ThemedText>
              {displayLockoutWarning ? (<ThemedText variant="popupModalText" style={style.modalText}>
                  {t('PINEnter.AttemptLockoutWarning')}
                </ThemedText>) : null}
            </>} onCallToActionLabel={t('Global.Okay')} onCallToActionPressed={clearAlertModal}/>) : null}
      {forgotPINModalVisible ? (<PopupModal notificationType={InfoBoxType.Info} title={t('PINEnter.ForgotPINModalTitle')} bodyContent={<ThemedText variant="popupModalText" style={style.modalText} testID={testIdWithKey('ForgotPINModalDescription')}>
              {t('PINEnter.ForgotPINModalDescription')}
            </ThemedText>} onCallToActionLabel={t('Global.Okay')} onCallToActionPressed={() => setForgotPINModalVisible(false)}/>) : null}
