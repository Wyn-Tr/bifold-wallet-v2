import { ProofMetadata, linkProofWithTemplate, sendProofRequest } from '@bifold/verifier';
...
import { BackHandler, DeviceEventEmitter, ScrollView, Share, StyleSheet, Vibration, View, useWindowDimensions, } from 'react-native';
...
const ProofRequesting = ({ route, navigation }) => {
    if (!route?.params) {
    ...
    const createProofRequest = useCallback(async () => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1038'), t('Error.Message1038'), e?.message ?? e, 1038);
    ...
    useEffect(() => {
        if (message && store.preferences.enableShareableLink) {
            const scanShareUrl = () => (<IconButton buttonLocation={ButtonLocation.Right} accessibilityLabel={t('Global.Share')} testID={testIdWithKey('ShareButton')} onPress={() => {
                    Share.share({ message });
                }} icon="share-variant"/>);
    ...
    return (<SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
      ...
      <View style={styles.buttonContainer}>
        <Button title={t('Verifier.RefreshQR')} accessibilityLabel={t('Verifier.RefreshQR')} testID={testIdWithKey('GenerateNewQR')} buttonType={ButtonType.Primary} onPress={() => createProofRequest()} disabled={generating}/>
      </View>
    </SafeAreaView>);
};
