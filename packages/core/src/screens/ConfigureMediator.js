...
const ConfigureMediator = ({ route }) => {
    ...
    const handleMediatorChange = async (mediatorId) => {
        if (mediatorId === store.preferences.selectedMediator)
            return;
        setPendingMediatorId(mediatorId);
        setIsModalVisible(true);
    };
    const MediatorRow = ({ label, id, testID, selected, onPress, }) => (<View style={[styles.section, styles.sectionRow]}>
      <ThemedText variant="title">{label}</ThemedText>
      <Pressable style={styles.checkboxContainer} accessibilityLabel={label} accessibilityRole="radio" testID={testIdWithKey(testID)}>
        <BouncyCheckbox disableText fillColor={ColorPalette.brand.secondaryBackground} unfillColor={ColorPalette.brand.secondaryBackground} size={36} innerIconStyle={{ borderColor: ColorPalette.brand.primary, borderWidth: 2 }} ImageComponent={() => <Icon name="circle" size={18} color={ColorPalette.brand.primary}/>} onPress={() => onPress(id)} isChecked={selected} disableBuiltInState/>
      </Pressable>
    </View>);
    return (<SafeAreaView style={styles.container}>
      <FlatList data={mediators} keyExtractor={(item) => item.id} renderItem={({ item }) => (<MediatorRow label={item.label.split('?')[0]} id={item.id} testID={item.testID} selected={store.preferences.selectedMediator === item.id} onPress={handleMediatorChange}/>)}/>
      {isModalVisible && (<DismissiblePopupModal title={t('Settings.ChangeMediator')} description={t('Settings.ChangeMediatorDescription')} onCallToActionLabel={t('Global.Confirm')} onCallToActionPressed={() => confirmMediatorChange()} onDismissPressed={() => setIsModalVisible(false)}/>)}
    </SafeAreaView>);
};
