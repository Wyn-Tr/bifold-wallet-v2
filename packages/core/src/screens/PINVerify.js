...
export var PINEntryUsage;
(function (PINEntryUsage) {
...
const PINVerify = ({ setAuthenticated, usage = PINEntryUsage.PINCheck, onCancelAuth }) => {
    ...
    return (<ScreenWrapper keyboardActive>
      <View style={style.screenContainer}>
        <View>
          {usage === PINEntryUsage.ChangeBiometrics && (<ThemedText variant="headingTwo" style={style.changeBiometricsHeader}>
          ...
          <ThemedText variant="bold" style={style.inputLabelText}>
            ...
            {usage === PINEntryUsage.ChangeBiometrics && (<ThemedText variant="caption">
                {` `}
                {t('PINEnter.ChangeBiometricsInputLabelParenthesis')}
              </ThemedText>)}
          </ThemedText>
          <PINInput onPINChanged={async (userPinInput) => {
            setPIN(userPinInput);
            if (userPinInput.length === minPINLength) {
                Keyboard.dismiss();
                usage === PINEntryUsage.ChangePIN && (await onPINInputCompleted(userPinInput));
            }
        }} testID={testIdWithKey(inputTestId[usage])} accessibilityLabel={inputLabelText[usage]} autoFocus={true} inlineMessage={inlineMessageField} onSubmitEditing={async (userPinInput) => {
            await onPINInputCompleted(userPinInput);
        }}/>
        </View>
        {usage === PINEntryUsage.ChangePIN && loading && (<View style={style.loadingContainer}>
            <LoadingSpinner size={50} color={ColorPalette.brand.primary}/>
          </View>)}
        {usage !== PINEntryUsage.ChangePIN && (<View style={style.buttonContainer}>
            <Button title={primaryButtonText[usage]} buttonType={ButtonType.Primary} testID={testIdWithKey(primaryButtonTestId[usage])} disabled={isContinueDisabled} accessibilityLabel={primaryButtonText[usage]} onPress={async () => {
                await onPINInputCompleted();
            }}>
              {loading && <ButtonLoading />}
            </Button>
          </View>)}
        {usage === PINEntryUsage.PINCheck && (<View style={[style.buttonContainer, { marginTop: 10 }]}>
            <Button title={t('PINEnter.AppSettingCancel')} buttonType={ButtonType.Secondary} testID={testIdWithKey('AppSettingCancel')} accessibilityLabel={t('PINEnter.AppSettingCancel')} onPress={() => onCancelAuth?.(false)}/>
          </View>)}
      </View>
      {alertModalVisible && (<PopupModal notificationType={InfoBoxType.Info} title={t('PINEnter.IncorrectPIN')} onCallToActionLabel={t('Global.Okay')} onCallToActionPressed={clearAlertModal}/>)}
    </ScreenWrapper>);
};
