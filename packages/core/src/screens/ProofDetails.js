...
const VerifiedProof = ({ record, isHistory, senderReview, connectionLabel, onBackPressed, onGenerateNewPressed, }) => {
    ...
    if (isHistory) {
        return (<ScrollView style={{ flexGrow: 1 }} testID={testIdWithKey('ProofDetailsHistoryView')}>
        <View style={styles.container}>
          {sharedProofDataItems.length > 0 && (<View style={styles.descriptionContainer}>
              <ThemedText>
                {senderReview ? (<>
    ...
    return (<ScrollView contentContainerStyle={{ flexGrow: 1 }} testID={testIdWithKey('ProofDetailsView')}>
      <View style={styles.container}>
        ...
        <View style={styles.footerButton}>
          <View style={{ marginBottom: 15 }}>
            <Button title={t('Verifier.GenerateNewQR')} accessibilityLabel={t('Verifier.GenerateNewQR')} testID={testIdWithKey('GenerateNewQR')} buttonType={ButtonType.Primary} onPress={onGenerateNewPressed}/>
          </View>
          <Button title={t('Verifier.BackToList')} accessibilityLabel={t('Verifier.BackToList')} testID={testIdWithKey('BackToList')} buttonType={ButtonType.Secondary} onPress={onBackPressed}/>
        </View>
      </View>
    </ScrollView>);
};
const UnverifiedProof = ({ record, onBackPressed, onGenerateNewPressed }) => {
    const { t } = useTranslation();
    ...
    return (<ScrollView contentContainerStyle={{ flexGrow: 1 }} testID={testIdWithKey('UnverifiedProofView')}>
      <View style={styles.header}>
        <View style={styles.headerTitleContainer}>
          {record.state === ProofState.Abandoned && (<ThemedText variant="bold" style={styles.headerTitle}>
              {t('ProofRequest.ProofRequestDeclined')}
            </ThemedText>)}
          {record.isVerified === false && (<ThemedText variant="bold" style={styles.headerTitle}>
...
const ProofDetails = ({ route, navigation }) => {
    if (!route?.params) {
    ...
    const connectionLabel = useMemo(() => connection
        ? getConnectionName(connection, store.preferences.alternateContactNames)
        : t('Verifier.ConnectionLessLabel'), [connection, store.preferences.alternateContactNames, t]);
    const cleanup = useCallback(() => {
        ...
        if (record &&
            record.connectionId &&
            (record.metadata.get(ProofMetadata.customMetadata).delete_conn_after_seen ||
    ...
    return (<SafeAreaView style={{ flexGrow: 1 }} edges={['left', 'right']}>
      {(record.isVerified || senderReview) && (<VerifiedProof record={record} isHistory={isHistory} senderReview={senderReview} connectionLabel={connectionLabel} onBackPressed={onBackPressed} onGenerateNewPressed={onGenerateNewPressed}/>)}
      {!(record.isVerified || senderReview) && (<UnverifiedProof record={record} onBackPressed={onBackPressed} onGenerateNewPressed={onGenerateNewPressed}/>)}
    </SafeAreaView>);
};
