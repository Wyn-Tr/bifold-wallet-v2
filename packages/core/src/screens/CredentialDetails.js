...
const CredentialDetails = ({ navigation, route }) => {
    ...
    const [isRevokedMessageHidden, setIsRevokedMessageHidden] = useState(credential?.metadata.get(CredentialMetadata.customMetadata)
        ?.revoked_detail_dismissed ?? false);
    const [store] = useStore();
    useEffect(() => {
        setIsRevokedMessageHidden(credential?.metadata.get(CredentialMetadata.customMetadata)
            ?.revoked_detail_dismissed ?? false);
    }, [credential?.metadata]);
    useEffect(() => {
    ...
    const containerBackgroundColor = overlay.brandingOverlay?.secondaryBackgroundColor && overlay.brandingOverlay.secondaryBackgroundColor !== ''
    ...
    useEffect(() => {
        ...
        bundleResolver.resolveAllBundles(params).then((bundle) => {
            setOverlay((o) => ({
                ...o,
                ...bundle,
                presentationFields: bundle.presentationFields?.filter((field) => field.value),
    ...
    useEffect(() => {
        ...
        const restoreMetadata = async () => {
            try {
                ...
                logger?.warn('Failed to restore credential metadata', { error: error });
            }
        };
        restoreMetadata();
    ...
    const callSubmitRemove = useCallback(async () => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1032'), t('Error.Message1032'), err?.message ?? err, 1032);
    ...
    const CredentialRevocationMessage = ({ credential }) => {
        return (<InfoBox notificationType={InfoBoxType.Error} title={t('CredentialDetails.CredentialRevokedMessageTitle') + ' ' + revocationDate} description={credential?.revocationNotification?.comment
                ? credential.revocationNotification.comment
                : t('CredentialDetails.CredentialRevokedMessageBody')} onCallToActionLabel={t('Global.Dismiss')} onCallToActionPressed={callDismissRevokedMessage}/>);
    };
    const getCredentialTop = () => {
        ...
        return (<View>
        <CredentialDetailSecondaryHeader overlay={overlay} brandingOverlayType={bundleResolver.getBrandingOverlayType()}/>
        <TouchableOpacity accessibilityLabel={`${overlay.metaOverlay?.watermark ? overlay.metaOverlay.watermark + '. ' : ''}${t('Credentials.IssuedBy')} ${overlay.metaOverlay?.issuer}`} accessibilityRole="button" accessibilityHint={t('CredentialDetails.NavigateToIssuerDetailsHint')} onPress={navigateToContactDetails} style={{ padding: 16, overflow: 'hidden' }}>
          {overlay.metaOverlay?.watermark && (<CardWatermark width={width} height={height} watermark={overlay.metaOverlay?.watermark}/>)}
        ...
        <View style={{ backgroundColor: ColorPalette.brand.secondaryBackground }}>
          <CredentialDetailPrimaryHeader overlay={overlay} brandingOverlayType={bundleResolver.getBrandingOverlayType()} credential={credential}/>
        </View>
      </View>);
    };
    const header = () => {
        return bundleResolver.getBrandingOverlayType() === BrandingOverlayType.Branding01 ? (<View>
        {isRevoked && !isRevokedMessageHidden ? (<View style={{ padding: paddingVertical, paddingBottom: 0 }}>
        ...
        {getCredentialTop()}
        {isRevoked && !isRevokedMessageHidden ? (<View style={{
    ...
    const footer = () => {
        ...
        {credentialConnectionLabel && isBranding10 ? (<View style={{
                    backgroundColor: ColorPalette.brand.secondaryBackground,
            ...
            {store?.preferences.developerModeEnabled && credential?.createdAt ? (<ThemedText testID={testIdWithKey('IssuedDate')}>
                <ThemedText variant="title" style={isRevoked && { color: ColorPalette.grayscale.mediumGrey }}>
        ...
        {store?.preferences.developerModeEnabled && (<View style={{
                    backgroundColor: ColorPalette.brand.secondaryBackground,
                    marginTop: paddingVertical,
                    paddingHorizontal,
                    paddingVertical,
                }}>
            <TouchableOpacity onPress={callViewJSONDetails} accessibilityLabel={t('Global.ViewJSON')} accessibilityRole={'button'} testID={testIdWithKey('JSONDetails')} style={{ flexDirection: 'row', gap: 8 }}>
              <Assets.svg.iconCode width={20} height={20} color={ColorPalette.brand.secondary}/>
              <ThemedText>{t('Global.ViewJSON')}</ThemedText>
            </TouchableOpacity>
          </View>)}
        {isRevoked ? (<View style={{
            ...
            <ThemedText style={{ color: ColorPalette.notification.errorText, marginTop: paddingVertical }} testID={testIdWithKey('RevocationMessage')}>
              {credential?.revocationNotification?.comment
                    ? credential.revocationNotification.comment
    ...
    return (<SafeAreaView style={{ flexGrow: 1 }} edges={['left', 'right']}>
      <Record fields={overlay.presentationFields || []} hideFieldValues header={header} footer={footer}/>
      <CommonRemoveModal usage={ModalUsage.CredentialRemove} visible={isRemoveModalDisplayed} onSubmit={callSubmitRemove} onCancel={callCancelRemove}/>
    </SafeAreaView>);
};
