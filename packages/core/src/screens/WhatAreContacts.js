...
const WhatAreContacts = ({ navigation }) => {
    ...
    const bulletPoints = [
        ...
        return (<View key={index} style={{ marginBottom: 10, flexDirection: 'row' }}>
        <ThemedText style={{ paddingRight: 5 }}>{'\u2022'}</ThemedText>
        <ThemedText style={{ flexShrink: 1 }}>{text}</ThemedText>
      </View>);
    });
    return (<SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.pageContent} directionalLockEnabled automaticallyAdjustContentInsets={false} showsHorizontalScrollIndicator={false}>
