...
const ProofRequestUsageHistoryRecord = ({ record, navigation }) => {
    ...
    return (<TouchableOpacity style={style.card} onPress={onDetails} disabled={!presentationReceived} accessibilityLabel={t('Screens.ProofDetails')} accessibilityRole="button" testID={testIdWithKey('ProofDetails')}>
      <View style={style.leftContainer}>
        ...
        <View style={style.cardRow}>
          ...
          <ThemedText style={style.valueText}>{t(getPresentationStateLabel(record))}</ThemedText>
        </View>
      </View>
...
const ProofRequestUsageHistory = ({ route, navigation }) => {
    if (!route?.params) {
    ...
    return (<SafeAreaView style={style.container} edges={['left', 'right']}>
      <FlatList data={proofs} keyExtractor={(proof) => proof.id} renderItem={({ item }) => <ProofRequestUsageHistoryRecord record={item} navigation={navigation}/>} ListEmptyComponent={() => <EmptyList />}/>
    </SafeAreaView>);
};
