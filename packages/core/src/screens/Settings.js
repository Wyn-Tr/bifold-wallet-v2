...
const Settings = ({ navigation }) => {
    ...
    const currentLanguage = i18n.t('Language.code', { context: i18n.language });
        ...
        },
        {
            ...
            data: [
                ...
                },
                {
                    ...
                    value: store.preferences.autoLockTime !== AutoLockTime.Never ? `${store.preferences.autoLockTime} min` : 'Never',
            ...
            data: [
                {
                    ...
                    onPress: () => navigation.getParent()?.navigate(Stacks.ProofRequestsStack, {
    ...
    if (store.preferences.useConnectionInviterCapability) {
        ...
        if (section) {
            section.data.splice(1, 0, {
                ...
                onPress: () => navigation.getParent()?.navigate(Stacks.ConnectStack, {
                    screen: Screens.Scan,
    ...
    const SectionHeader = ({ icon, iconRight, title, titleTestID }) => 
    // gate keep behind developer mode
    store.preferences.useConnectionInviterCapability ? (<View style={[styles.section, styles.sectionHeader, { justifyContent: iconRight ? 'space-between' : undefined }]}>
        <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
          <Icon importantForAccessibility={'no-hide-descendants'} accessible={false} name={icon.name} size={icon.size ?? defaultIconSize} style={[{ marginRight: 10, color: SettingsTheme.iconColor }, icon.style]}/>
          <ThemedText variant="headingThree" testID={titleTestID} numberOfLines={1} accessibilityRole={'header'} style={{ flexShrink: 1 }}>
            {title}
          </ThemedText>
        </View>
        {iconRight && (<IconButton buttonLocation={ButtonLocation.Right} accessibilityLabel={iconRight.accessibilityLabel} testID={iconRight.testID} onPress={iconRight.action} icon={'pencil'} iconTintColor={TextTheme.headingThree.color}/>)}
      </View>) : (<View style={[styles.section, styles.sectionHeader]}>
        <Icon importantForAccessibility={'no-hide-descendants'} accessible={false} name={icon.name} size={24} style={{ marginRight: 10, color: SettingsTheme.iconColor }}/>
        <ThemedText maxFontSizeMultiplier={1.8} variant="headingThree" accessibilityRole={'header'} style={{ flexShrink: 1 }}>
          {title}
        </ThemedText>
      </View>);
    const SectionRow = ({ title, value, accessibilityLabel, testID, onPress }) => (<ScrollView horizontal style={styles.section} contentContainerStyle={{ flexGrow: 1 }}>
      <TouchableOpacity accessible={true} accessibilityLabel={accessibilityLabel} accessibilityRole="button" testID={testID} style={styles.sectionRow} onPress={onPress}>
        <ThemedText style={[TextTheme.settingsText, { marginRight: 14, maxWidth: fontIsGreaterThanCap ? '95%' : '100%' }]}>
          {title}
        </ThemedText>
    ...
    return (<SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
      <SectionList renderItem={({ item: { title, value, accessibilityLabel, testID, onPress } }) => (<SectionRow title={title} accessibilityLabel={accessibilityLabel} testID={testID ?? 'NoTestIdFound'} value={value} onPress={onPress}/>)} renderSectionHeader={({ section: { header: { title, icon, iconRight, titleTestID }, }, }) => <SectionHeader icon={icon} iconRight={iconRight} title={title} titleTestID={titleTestID}/>} ItemSeparatorComponent={() => (<View style={{ backgroundColor: SettingsTheme.groupBackground }}>
            <View style={styles.itemSeparator}></View>
          </View>)} SectionSeparatorComponent={() => <View style={styles.sectionSeparator}></View>} ListFooterComponent={() => (<View style={styles.footer}>
            <TouchableWithoutFeedback onPress={incrementDeveloperMenuCounter} disabled={store.preferences.developerModeEnabled}>
              <View>
          ...
          </View>)} sections={settingsSections} stickySectionHeadersEnabled={false}></SectionList>
    </SafeAreaView>);
};
