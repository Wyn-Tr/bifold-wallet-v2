...
const CredentialOffer = ({ navigation, credentialId }) => {
    const { agent } = useAppAgent();
    const { t, i18n } = useTranslation();
    const { ColorPalette } = useTheme();
    const { RecordLoading } = useAnimatedComponents();
    const { assertNetworkConnected } = useNetwork();
    const [bundleResolver, { enableTours: enableToursConfig }, logger, historyManagerCurried, historyEnabled, historyEventsLogger,] = useServices([
    ...
    const [overlay, setOverlay] = useState({ presentationFields: [] });
    ...
    const handleAcceptTouched = useCallback(async () => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1024'), t('Error.Message1024'), err?.message ?? err, 1024);
    ...
    const handleDeclineTouched = useCallback(async () => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1025'), t('Error.Message1025'), err?.message ?? err, 1025);
            DeviceEventEmitter.emit(EventTypes.ERROR_ADDED, error);
        }
    }, [
        agent,
    ...
    const header = () => {
        ...
        {!loading && credential && (<View style={{ marginHorizontal: 15, marginBottom: 16 }}>
    ...
    const footer = () => {
        ...
        {Boolean(credentialConnectionLabel) && goalCode === 'aries.vc.issue' && (<ConnectionAlert connectionLabel={credentialConnectionLabel}/>)}
        <View style={styles.footerButton}>
          <Button title={t('Global.Accept')} accessibilityLabel={t('Global.Accept')} testID={testIdWithKey('AcceptCredentialOffer')} buttonType={ButtonType.Primary} onPress={handleAcceptTouched} disabled={!buttonsVisible}/>
        </View>
        <View style={styles.footerButton}>
          <Button title={t('Global.Decline')} accessibilityLabel={t('Global.Decline')} testID={testIdWithKey('DeclineCredentialOffer')} buttonType={ButtonType.Secondary} onPress={toggleDeclineModalVisible} disabled={!buttonsVisible}/>
        </View>
      </View>);
    };
    return (<SafeAreaView style={{ flexGrow: 1 }} edges={['bottom', 'left', 'right']}>
      ...
      <CommonRemoveModal usage={ModalUsage.CredentialOfferDecline} visible={declineModalVisible} onSubmit={handleDeclineTouched} onCancel={toggleDeclineModalVisible}/>
    </SafeAreaView>);
};
