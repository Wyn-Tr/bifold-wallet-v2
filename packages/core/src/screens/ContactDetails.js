...
const ContactDetails = ({ route }) => {
    ...
    const [{ contactDetailsOptions }, ContactCredentialListItem, logger, historyManagerCurried, historyEnabled, historyEventsLogger,] = useServices([
        TOKENS.CONFIG,
    ...
    const callSubmitRemove = useCallback(async () => {
        try {
            ...
            const error = new BifoldError(t('Error.Title1037'), t('Error.Message1037'), err?.message ?? err, 1037);
    ...
    const callStartVideoCall = useCallback(() => {
        navigation.navigate(Screens.VideoCall, { connectionId, video: true });
    }, [navigation, connectionId]);
    ...
    const contactImage = () => {
        return (<>
        {contactImageUrl ? (<View style={styles.contactImgContainer}>
            <Image style={styles.contactImg} source={toImageSource(contactImageUrl)}/>
          </View>) : (<View style={styles.contactFirstLetterContainer}>
            <ThemedText allowFontScaling={false} variant="bold" accessible={false} style={{
    ...
    return (<SafeAreaView style={{ flexGrow: 1 }} edges={['bottom', 'left', 'right']}>
      <View style={[styles.contentContainer, contactDetailsOptions?.enableCredentialList && { flex: 2 }]}>
        ...
        {contactDetailsOptions?.showConnectedTime && (<ThemedText style={{ marginTop: 20 }}>
        ...
        {contactDetailsOptions?.enableCredentialList && (<>
            ...
            <FlatList ItemSeparatorComponent={() => <View style={{ height: 20 }}></View>} ListEmptyComponent={() => (<ThemedText style={{ color: ColorPalette.grayscale.lightGrey }}>
                  {t('ContactDetails.NoCredentials')}
                </ThemedText>)} data={connectionCredentials} renderItem={({ item }) => (<ContactCredentialListItem credential={item} onPress={() => navigation.navigate(Screens.CredentialDetails, { credentialId: item.id })}/>)} keyExtractor={(item) => item.id}/>
          </>)}
      </View>
      <View>
        {/* Video Call Button */}
        <TouchableOpacity onPress={callStartVideoCall} accessibilityLabel={t('ContactDetails.StartVideoCall')} accessibilityRole={'button'} testID={testIdWithKey('StartVideoCall')} style={[styles.contentContainer, styles.actionContainer, { marginTop: 10 }]}>
        ...
        {contactDetailsOptions?.enableEditContactName && (<TouchableOpacity onPress={callGoToRename} accessibilityLabel={t('Screens.RenameContact')} accessibilityRole={'button'} testID={testIdWithKey('RenameContact')} style={[styles.contentContainer, styles.actionContainer, { marginTop: 10 }]}>
        ...
        {store?.preferences.developerModeEnabled && (<TouchableOpacity onPress={callViewJSONDetails} accessibilityLabel={t('Global.ViewJSON')} accessibilityRole={'button'} testID={testIdWithKey('JSONDetails')} style={[styles.contentContainer, styles.actionContainer, { marginTop: 10 }]}>
            <Assets.svg.iconCode width={20} height={20} color={ColorPalette.brand.text}/>
            <ThemedText>{t('Global.ViewJSON')}</ThemedText>
          </TouchableOpacity>)}
        <TouchableOpacity onPress={callOnRemove} accessibilityLabel={t('ContactDetails.RemoveContact')} accessibilityRole={'button'} testID={testIdWithKey('RemoveFromWallet')} style={[styles.contentContainer, styles.actionContainer, { marginTop: 10 }]}>
      ...
      <CommonRemoveModal usage={ModalUsage.ContactRemove} visible={isRemoveModalDisplayed} onSubmit={callSubmitRemove} onCancel={callCancelRemove}/>
      <CommonRemoveModal usage={ModalUsage.ContactRemoveWithCredentials} visible={isCredentialsRemoveModalDisplayed} onSubmit={callGoToCredentials} onCancel={callCancelUnableToRemove}/>
    </SafeAreaView>);
};
