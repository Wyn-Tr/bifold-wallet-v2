...
export const buildFieldsFromW3cCredsCredential = (display, filterByAttributes) => {
    return (Object.entries(display.attributes)
        .filter(([key]) => key !== 'id' && key !== 'sub' && key !== 'status')
        .map(([key]) => getAttributeField(display, key))
        .filter((field) => field !== undefined)
        .filter((field) => (filterByAttributes ? filterByAttributes.includes(field.attribute_name) : true))
        .map((fld) => fld.field) || []);
};
...
export const buildOverlayFromW3cCredential = async ({ credentialDisplay, language, resolver, }) => {
    const params = {
        identifiers: {
            schemaId: '',
