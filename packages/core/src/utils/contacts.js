 ...
 */
export const fetchContactsByLatestMessage = async (agent, connections) => {
    const connectionsWithMessages = await Promise.all(connections.map(async (conn) => ({
        conn,
        msgs: [
    ...
    const connectionsWithLatestMessage = connectionsWithMessages.map((pair) => {
        return {
            conn: pair.conn,
            latestMsg: pair.msgs.reduce((acc, cur) => {
            ...
            { createdAt: pair.conn.createdAt }),
        };
    });
    return connectionsWithLatestMessage
        .sort((a, b) => new Date(b.latestMsg.updatedAt || b.latestMsg.createdAt).valueOf() -
