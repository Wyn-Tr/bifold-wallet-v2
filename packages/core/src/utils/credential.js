...
async function resolveIdsFromFormatData(credential, agent, logger) {
    try {
        ...
        logger?.error('Failed to get format data', { error: error });
    }
    return {};
}
/**
...
async function resolveSchemaName(schemaId, agent, logger) {
    try {
        ...
        logger?.warn('Failed to resolve schema', { error: error, schemaId });
        return undefined;
    }
}
/**
...
async function resolveCredDefTag(credDefId, agent, logger) {
    try {
        ...
        logger?.warn('Failed to resolve credential definition', { error: error, credDefId });
        return undefined;
    }
}
/**
...
export async function ensureCredentialMetadata(credential, agent, offerData, logger) {
    ...
    const { schemaId, credDefId } = await determineSchemaAndCredDefIds(credential, agent, offerData, existingMetadata, logger);
    ...
    const schemaName = needsSchemaName && schemaId ? await resolveSchemaName(schemaId, agent, logger) : existingMetadata?.schemaName;
    const credDefTag = needsCredDefTag && credDefId ? await resolveCredDefTag(credDefId, agent, logger) : existingMetadata?.credDefTag;
