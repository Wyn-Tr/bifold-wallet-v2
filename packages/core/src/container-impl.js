...
import { createActionMenuHandler, createBasicMessageHandler, createChatScreenConfig, createCredentialHandler, createDIDCommWorkflowHandler, createProofHandler, WorkflowRegistry, } from './modules/workflow';
    ...
    init() {
        ...
        this._container.registerInstance(TOKENS.LOAD_STATE, async (dispatch) => {
            ...
            await Promise.all([
                ...
                loadState(LocalStorageKeys.Preferences, (val) => (preferences = val)),
                loadState(LocalStorageKeys.Migration, (val) => (migration = val)),
                loadState(LocalStorageKeys.Tours, (val) => (tours = val)),
                loadState(LocalStorageKeys.Onboarding, (val) => (onboarding = val)),
            ]);
            const state = {
        ...
        workflowRegistry.setChatScreenConfig(createChatScreenConfig({
            header: {
...
export const SystemContext = createContext(undefined);
