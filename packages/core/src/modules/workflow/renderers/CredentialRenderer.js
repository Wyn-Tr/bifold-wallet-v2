 ...
 */
import { CredentialPreviewAttribute, CredentialState } from '@credo-ts/core';
 ...
 */
export var CredentialDisplayType;
(function (CredentialDisplayType) {
    CredentialDisplayType["STUDENT_ID"] = "student_id";
    CredentialDisplayType["TRANSCRIPT"] = "transcript";
    CredentialDisplayType["DEFAULT"] = "default";
})(CredentialDisplayType || (CredentialDisplayType = {}));
/**
 * Detect credential type from credential definition ID or attributes
 */
export function detectCredentialType(credential) {
    const credDefId = credential.metadata?.data?.['_anoncreds/credential']?.credentialDefinitionId || '';
    const credentialAttributes = credential.credentialAttributes || [];
    // Check for transcript attributes
    const hasGPA = credentialAttributes.some((attr) => attr.name.toLowerCase().includes('gpa') ||
        attr.name.toLowerCase().includes('termgpa') ||
        attr.name.toLowerCase().includes('cumulativegpa'));
    const hasYearStart = credentialAttributes.some((attr) => attr.name.toLowerCase() === 'yearstart' || attr.name.toLowerCase() === 'year_start');
    ...
    const hasStudentId = credentialAttributes.some((attr) => attr.name.toLowerCase() === 'studentid' ||
        attr.name.toLowerCase() === 'studentnumber' ||
        attr.name.toLowerCase() === 'student_id');
    const hasStudentName = credentialAttributes.some((attr) => attr.name.toLowerCase() === 'fullname' ||
 ...
 */
function useCredentialAttributes(credential) {
    const { agent } = useAgent();
    const [attributes, setAttributes] = useState(credential.credentialAttributes || []);
    const [loading, setLoading] = useState(false);
    const [credDefId, setCredDefId] = useState(credential.metadata?.data?.['_anoncreds/credential']?.credentialDefinitionId || '');
    useEffect(() => {
 ...
 */
export const DefaultCredentialCard = ({ credential, context, onPress }) => {
    ...
    const fullName = credentialAttributes.find((attr) => attr.name.toLowerCase() === 'fullname' || attr.name.toLowerCase() === 'studentfullname')?.value;
    ...
    const studentId = credentialAttributes.find((attr) => attr.name.toLowerCase() === 'studentid' || attr.name.toLowerCase() === 'studentnumber')?.value;
    ...
    const allAttributes = credentialAttributes.filter((attr) => !['studentphoto', 'photo', 'student_photo'].includes(attr.name.toLowerCase()));
    const content = (<View style={[styles.card, { backgroundColor: SettingsTheme.newSettingColors.bgColorUp || '#1a2634' }]}>
      {/* Header with state */}
      ...
      <View style={styles.body}>
        {loading ? (<View style={styles.loadingContainer}>
            ...
            {school && (<ThemedText style={[styles.school, { color: SettingsTheme.newSettingColors.headerTitle }]}>
                {school}
              </ThemedText>)}

            {/* Show display name if available */}
            {displayName && (<ThemedText style={[styles.name, { color: SettingsTheme.newSettingColors.textBody }]}>
                {displayName}
              </ThemedText>)}

            {/* Show student ID if available */}
            {studentId && (<ThemedText style={[styles.detail, { color: SettingsTheme.newSettingColors.textColor }]}>
                {context.t('Chat.StudentID')}: {studentId}
              </ThemedText>)}
            ...
            {!school &&
                !studentId &&
                !displayName &&
                allAttributes.slice(0, 4).map((attr, index) => (<ThemedText key={index} style={[styles.detail, { color: SettingsTheme.newSettingColors.textColor }]}>
                  {attr.name && attr.value ? `${attr.name}: ${attr.value}` : ''}
                </ThemedText>))}

            {/* Show tap to view message */}
            {allAttributes.length > 0 && (<ThemedText style={[styles.tapToView, { color: SettingsTheme.newSettingColors.textColor }]}>
                {context.t('Chat.TapToView') || 'Tap to view details'}
              </ThemedText>)}
          </>)}
      </View>
 ...
 */
export class DefaultCredentialRenderer {
    options;
    constructor(options = {}) {
        this.options = options;
    }
    render(credential, context) {
        ...
        return (<View>
        <VDCredentialCard credential={credential} context={context} onPress={handlePress} onAccept={handleAccept} onDecline={handleDecline}/>
      </View>);
    }
}
/**
 ...
 */
export function createDefaultCredentialRenderer(options = {}) {
...
function _getAttributeValue(credential, ...names) {
 ...
 */
export const VDCredentialCard = ({ credential, context, onPress, onAccept, onDecline, }) => {
    const { SettingsTheme } = useTheme();
    ...
    if (loading) {
        return (<View style={[styles.card, { backgroundColor: SettingsTheme.newSettingColors.bgColorUp || '#1a2634' }]}>
        <View style={[styles.header, { backgroundColor: SettingsTheme.newSettingColors.buttonColor }]}>
    ...
    const renderCard = () => {
        switch (credentialType) {
            case CredentialDisplayType.STUDENT_ID:
                return (<VDCard firstName={firstName} lastName={lastName} fullName={fullName} studentId={studentId} school={school} issueDate={issueDate} credDefId={credDefId} issuerName={context.theirLabel} isInChat={context.isInChat} studentPhoto={studentPhoto}/>);
            case CredentialDisplayType.TRANSCRIPT:
                return (<TranscriptCard school={school} yearStart={yearStart} yearEnd={yearEnd} termGPA={termGPA} cumulativeGPA={cumulativeGPA} fullname={fullName || `${firstName} ${lastName}`} isInChat={context.isInChat}/>);
            default:
    ...
    return (<View>
      {onPress ? (<TouchableOpacity onPress={handlePress} activeOpacity={0.8}>
          {cardContent}
        </TouchableOpacity>) : (cardContent)}

      {/* Render action buttons if available */}
      {credential.state === CredentialState.OfferReceived && CredentialButtons && onAccept && onDecline && (<CredentialButtons isProcessing={false} onAccept={onAccept} onDecline={onDecline}/>)}
    </View>);
};
/**
 * Detect credential type from attributes array (for use after async loading)
 */
function detectCredentialTypeFromAttributes(attributes, credDefId) {
    // Check for transcript attributes
    const hasGPA = attributes.some((attr) => attr.name.toLowerCase().includes('gpa') ||
        attr.name.toLowerCase().includes('termgpa') ||
        attr.name.toLowerCase().includes('cumulativegpa'));
    const hasYearStart = attributes.some((attr) => attr.name.toLowerCase() === 'yearstart' || attr.name.toLowerCase() === 'year_start');
    if (hasGPA || hasYearStart || credDefId.toLowerCase().includes('transcript')) {
        return CredentialDisplayType.TRANSCRIPT;
    }
    // Check for student ID attributes
    const hasStudentId = attributes.some((attr) => attr.name.toLowerCase() === 'studentid' ||
        attr.name.toLowerCase() === 'studentnumber' ||
        attr.name.toLowerCase() === 'student_id');
    const hasStudentName = attributes.some((attr) => attr.name.toLowerCase() === 'fullname' ||
        attr.name.toLowerCase() === 'studentfullname' ||
        attr.name.toLowerCase() === 'first' ||
        attr.name.toLowerCase() === 'last');
    if (hasStudentId && hasStudentName) {
 ...
 */
export class VDCredentialRenderer {
    options;
    constructor(options = {}) {
        this.options = options;
    }
    render(credential, context) {
        ...
        return (<View style={styles.vdCard}>
        <VDCredentialCard credential={credential} context={context} onPress={handlePress} onAccept={handleAccept} onDecline={handleDecline}/>
      </View>);
    }
}
 ...
 */
export function createVDCredentialRenderer(options = {}) {
