    ...
    useEffect(() => {
        const getSavedHistorySettingsOption = async () => {
            ...
            if (storedHistorySettingsOption === 'Never') {
                ...
                setInitialHistory(storedHistorySettingsOption
      ...
      <View style={style.screenContainer}>
        <View style={style.contentContainer}>
          <View>
            ...
            <SingleSelectBlock initialSelect={initialHistory} selection={historyManager?.getHistorySettingsOptionList()} onSelect={onSelectHistory}/>
          </View>
        </View>
        <View style={style.controlsContainer}>
          <Button title={actionButtonLabel} testID={actionButtonTestId} accessibilityLabel={actionButtonLabel} buttonType={ButtonType.Primary} onPress={handleSaveHistorySettings}>
            {!continueEnabled && isLoading ? <ButtonLoading /> : null}
          </Button>
          <View style={{ marginBottom: 10 }}/>
          <Button title={t('ActivityHistory.StopKeepingHistory')} testID={actionButtonTestId} accessibilityLabel={actionButtonLabel} buttonType={ButtonType.Secondary} onPress={async () => {
