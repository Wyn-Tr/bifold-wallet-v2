...
export async function fetchInvitationDataUrl(dataUrl) {
 ...
 */
export const extractCertificateFromAuthorizationRequest = async ({ data, uri, }) => {
    try {
        if (data) {
            return {
                data,
...
export async function withTrustedCertificate(agent, certificate, method) {
    ...
    try {
        ...
        if (certificate)
            x509ModuleConfig.setTrustedCertificates(currentTrustedCertificates);
    }
}
//This settings should be moved to an injectable config
const allowUntrustedCertificates = false;
export const getCredentialsForProofRequest = async ({ agent, data, uri, }) => {
    let requestUri = uri;
    try {
        ...
        agent.config.logger.error(`Parsing presentation request:  ${err?.message ?? err}`);
        throw err;
    }
};
export const shareProof = async ({ agent, authorizationRequest, credentialsForRequest, selectedCredentials, allowUntrustedCertificate = false, }) => {
    ...
    const credentials = Object.fromEntries(credentialsForRequest.requirements.flatMap((requirement) => requirement.submissionEntry.map((entry) => {
        const credentialId = selectedCredentials[entry.inputDescriptorId].id;
        const credential = entry.verifiableCredentials.find((vc) => vc.credentialRecord.id === credentialId) ??
    ...
    try {
        ...
        throw new Error(`Error accepting proof request. ${error?.message ?? error}`);
    }
};
