...
const OpenIDProofCredentialSelect = ({ route, navigation }) => {
    if (!route?.params) {
    ...
    const listHeader = () => {
        return (<View style={{ ...styles.pageMargin, marginTop: 40, marginBottom: 20 }}>
        {loading ? (<View style={styles.cardLoading}>
            <RecordLoading />
          </View>) : (<ThemedText variant={'bold'}>{t('ProofRequest.AvailableCards')}</ThemedText>)}
      </View>);
    };
    return (<ScreenLayout screen={Screens.OpenIDProofCredentialSelect}>
      <FlatList data={credentialsRequested} ListHeaderComponent={listHeader} renderItem={({ item }) => {
            return (<View style={styles.pageMargin}>
              <TouchableOpacity accessibilityRole="button" testID={testIdWithKey(`select:${item.credential.id}`)} onPress={() => changeCred(item)} style={[
