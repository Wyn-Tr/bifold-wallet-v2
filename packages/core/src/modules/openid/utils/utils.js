...
export function getCredentialConfigurationIds(resolved) {
    const fromOffered = (resolved.offeredCredentials ?? []).map((c) => c?.id).filter((x) => !!x);
...
export function buildResolvedOfferFromMeta(meta) {
