...
const InvalidBadge = ({ isInvalid }) => {
    ...
    if (!isInvalid)
        return null;
    return (<View style={styles.badgeWrap} testID={testIdWithKey('CredentialInvalidBadge')}>
      <Text style={styles.badgeText}>Invalid</Text>
    </View>);
};
const OpenIDCredentialCard = ({ credentialDisplay, credentialRecord, style = {}, onPress = undefined, }) => {
    ...
    const display = useMemo(() => {
        if (credentialDisplay)
            return credentialDisplay.display;
        if (!credentialRecord) {
    ...
    const overlayAttributeField = useMemo(() => {
        if (!display?.primary_overlay_attribute || !credentialDisplay)
            return undefined;
    ...
    const logoContaineter = (logo) => {
        ...
        return (<Image source={toImageSource(src)} style={{
                flex: 4,
                resizeMode: 'contain',
                width: width,
                height: height,
    ...
    const CardHeader = () => {
        ...
        <View testID={testIdWithKey('CredentialCardHeader')} style={[styles.innerHeaderContainer]}>
          ...
          <View style={styles.innerHeaderCredInfoContainer}>
            <View>
              <Text numberOfLines={1} ellipsizeMode="tail" style={styles.titleFontCredentialName} testID={testIdWithKey('CredentialIssuer')} maxFontSizeMultiplier={1}>
                {display?.name}
              </Text>
            </View>
            <View>
              <Text numberOfLines={1} ellipsizeMode="tail" style={styles.titleFontCredentialDescription} testID={testIdWithKey('CredentialName')} maxFontSizeMultiplier={1}>
                {display?.description}
              </Text>
            </View>
          </View>
    ...
    return (<View>
      <TouchableOpacity accessible={true} accessibilityLabel={`${display?.issuer.name ? `${t('Credentials.IssuedBy')} ${display?.issuer.name}` : ''}, ${t('Credentials.Credential')}.`} accessibilityRole="button" disabled={typeof onPress === 'undefined' ? true : false} onPress={onPress} style={[styles.cardContainer, style]} testID={testIdWithKey('ShowCredentialDetails')}>
        <View style={[styles.flexGrow, { overflow: 'hidden' }]} testID={testIdWithKey('CredentialCard')}>
          {display?.backgroundImage ? (<ImageBackground source={toImageSource(display.backgroundImage.url)} style={styles.flexGrow} imageStyle={{ borderRadius }}>
              <CredentialCard />
            </ImageBackground>) : (<CredentialCard />)}
        </View>
      </TouchableOpacity>
    </View>);
};
