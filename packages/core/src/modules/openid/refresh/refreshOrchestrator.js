...
const defaultToLite = (rec) => ({
    ...
    format: (rec instanceof W3cCredentialRecord && ClaimFormat.JwtVc) ||
...
export class RefreshOrchestrator {
    ...
    constructor(logger, bridge, opts) {
        ...
        bridge.onReady((agent) => {
            ...
            if (this.opts.autoStart && this.opts.intervalMs)
                this.start();
        }, true);
    }
    configure(next) {
    ...
    start() {
        if (this.intervalOn || !this.opts.intervalMs)
            return;
    ...
    async checkRecordStatus(rec) {
        const { shouldSkip, markRefreshing, clearRefreshing, upsert, markInvalid, setLastSweep } = credentialRegistry.getState();
        const id = rec.id;
        if (!this.agent) {
    ...
    async refreshRecord(rec) {
        const { shouldSkip, markRefreshing, clearRefreshing, clearExpired, markExpiredWithReplacement, blockAsFailed, blockAsSucceeded, upsert, } = credentialRegistry.getState();
        const id = rec.id;
        if (!this.agent) {
            this.logger.error(`💥 [Refresh] Agent not initialized, cannot refresh credential ${id}`);
            return;
