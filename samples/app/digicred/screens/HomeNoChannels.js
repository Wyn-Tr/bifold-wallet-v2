...
import { View, Text, StyleSheet, Animated, TouchableOpacity, StatusBar, ActivityIndicator, Dimensions, } from 'react-native';
...
const HomeNoChannels = () => {
    ...
    const [shouldRenderHomeNoChannels, setShouldRenderHomeNoChannels] = useState(null);
    useEffect(() => {
        if (!loading && !hasChecked.current) {
            hasChecked.current = true;
            const hasCredential = credentials.some((c) => c.state === CredentialState.CredentialReceived || c.state === CredentialState.Done);
            const hasConnection = connections.some((conn) => {
                if (conn.connectionTypes.includes(ConnectionType.Mediator))
                    return false;
                const name = conn.theirLabel || conn.alias;
                if (contactHideList?.includes(name ?? ''))
                    return false;
    ...
    const contents = [
        {
            ...
            description: 'Digital credentials are the electronic equivalent of physical credentials and documents such as identity cards, certificates, or transcripts offered by participating services.\n\nServices are simplified and expedited as organizations can confirm who you are and what you have accomplished with trusted information from the digital credentials.',
        },
        {
        ...
        },
        {
            ...
            description: 'This is your wallet where you will hold your digital credentials. It will help you receive and present your documents when you are in-person or online. You will receive your documents from participating organizations and be able to safely and securely share them with others.\n\nInteract with confidence with individuals and organizations you trust.',
        },
    ];
    ...
    return (<GradientBackground>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={[styles.contentWrapper, { marginTop: isTablet() ? 180 : 0 }]}>
          <Animated.View style={[
            ...
            <LinearGradient colors={DigiCredColors.homeNoChannels.buttonGradient} locations={DigiCredColors.homeNoChannels.buttonGradientLocations} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.backgroundBtn, isTablet() ? { width: '60%', height: 60 } : { width: '75%' }]}>
              <DigiCredButton title={t('homeNoChannels.joinButton')} onPress={handleScanPress} variant="primary" customStyle={styles.button} customTextStyle={styles.buttonText} iconName="qrcode-scan" iconSize={20} iconColor={DigiCredColors.text.primary}/>
            </LinearGradient>
            <Text style={[
            styles.howItWorks,
            {
            ...
            styles.animatedItem,
            {
              ...
              <TouchableOpacity style={[styles.item, { width: itemWidth }]} onPress={() => handleItemPress(0)} activeOpacity={0.7}>
            ...
            styles.animatedItem,
            {
              ...
              <TouchableOpacity style={[styles.item, { width: itemWidth }]} onPress={() => handleItemPress(1)} activeOpacity={0.7}>
                <View style={styles.iconWrapper}>
            ...
            styles.animatedItem,
            {
              ...
              <TouchableOpacity style={[styles.item, { width: itemWidth }]} onPress={() => handleItemPress(2)} activeOpacity={0.7}>
                <View style={styles.iconWrapper}>
      ...
      {modalVisible && modalContent && (<HomeNoChannelModal visible={modalVisible} onClose={() => setModalVisible(false)} content={modalContent}/>)}
    </GradientBackground>);
};
